...
| 첫 화면 구조 | A+ 후보 | 평균 사용자가 첫 화면에서 주 행동과 상세 진입을 설명 없이 이해한다. | Active · 주요 이슈 story ring, 카드 primary `근거·영상`, 평면 `지도에서 보기` 행, 반복 감사 라벨 제거, 상단 숫자판 DOM 제거, 390/430/768/1440px 실제 렌더링 `overflowX=false` |
...
| 화면 | 파일 | 확인 |
|---|---|---|
| 상업용 재설계 74차 현재 evidence 세트 | `docs/visual-evidence/current/visual-surface-evidence.json` | `pnpm check:visual-surface:evidence`가 390/430/768/1440px의 홈·상세·영상·탐색·제보 20개 PNG를 생성. 현재 로컬 첫 이슈는 `정보통신망법 개정 관련 집회`, `sourceBundleFirst=false`, forbidden 0, `serviceSyncState=delayed`, 배너 `공개자료로 먼저 확인`. 영상 탭은 poster 없는 LIVE Claim을 큰 가짜 영상판이 아니라 `reel-pending-card` 검토 카드로 표시하고, `reelPendingFullCount=0`을 검사한다 |
| 상업용 재설계 74차 모바일 홈 | `docs/visual-evidence/current/mobile_390_home.png` | 390px 홈에서 story ring, 이슈 카드 2개, 하단 5탭, 배너 `공개자료로 먼저 확인`, 첫 카드 `서울 도심권 · 수도권 · 2026.07.12 기준`, `근거·영상/반론·정정` 경로 유지. API live 동기화 전이라 S+ 완료 증거는 아님 |
