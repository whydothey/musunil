...
| 현장 영상 UX | A 후보 | 영상이 확정 결론이 아니라 현장 인증 자료이며, 근거·위치 범위·관련 이슈에 접근한다. | Active · 33차로 poster 없는 LIVE Claim은 빈 릴스판 대신 검토 카드로 표시. 모바일 `posterImages=0`, `reviewSlots=0`, `reviewPanel=true`, `navOverlap=false`, 금지 문구 0. 실제 제보 영상 품질과 사용자 수락 필요 |
