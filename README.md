...
Render 배포가 끝난 뒤 로컬 셸 없이 검증하려면 GitHub Actions `post-deploy` 수동 workflow를 실행한다. `web-deploy` 모드는 strict Web header와 static hash를 확인하고, `final-gate` 모드는 `pnpm launch:final-gate` 전체를 실행한다. `final-gate`에서 Render API token을 workflow secret으로 쓰지 않는 경우에는 Render `musunil-api > Custom Domains > api.musunil.com`의 DNS target hostname을 `render_api_dns_target` 입력칸에 넣어 strict API CNAME 검증까지 같은 조건으로 실행한다. 이 workflow는 push에 자동 실행되지 않으므로 외부 DNS/API 연결 전 일반 커밋을 막지 않는다.
