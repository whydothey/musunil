...
법안 관련 뉴스 자동 수집에는 아래 NAVER API HUB 키가 별도로 필요하다.

- `public_data_sources.naver_api_hub_client_id`
- `public_data_sources.naver_api_hub_client_secret`
- 또는 Render cron의 `MUSUNIL_NAVER_API_HUB_CLIENT_ID`, `MUSUNIL_NAVER_API_HUB_CLIENT_SECRET`

기본값은 최근 90일, 검색당 20건, 실행당 최대 100개 쿼리이며 API가 월 20,000회 이후 호출을 차단한다. 키가 없으면 뉴스 작업만 `news_disabled`로 종료되고 법안·집회 데이터는 계속 갱신된다. 뉴스 검색 설명은 관련성 판정 후 저장하지 않는다.
