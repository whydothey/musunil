...
| 화면 | 파일 | 확인 |
|---|---|---|
| 상업용 재설계 73차 현재 evidence 세트 | `docs/visual-evidence/current/visual-surface-evidence.json` | `pnpm check:visual-surface:evidence`가 390/430/768/1440px의 홈·상세·영상·탐색·제보 20개 PNG를 생성. 현재 로컬 첫 이슈는 `정보통신망법 개정 관련 집회`, `sourceBundleFirst=false`, forbidden 0, `serviceSyncState=delayed`, 배너 `공개자료로 먼저 확인`. 홈은 `공개자료 기반 집회·시위`, 추상 story ring, 위치/근거 라벨 행, `근거·영상` primary, `지도에서 보기`, `반론·정정` 경로로 표시됨 |
| 상업용 재설계 73차 모바일 홈 | `docs/visual-evidence/current/mobile_390_home.png` | 390px 홈에서 story ring, 이슈 카드 2개, 하단 5탭, 배너 `공개자료로 먼저 확인`, 첫 카드 `서울 도심권 · 수도권 · 2026.07.12 기준`, `근거·영상/반론·정정` 경로 유지. API live 동기화 전이라 S+ 완료 증거는 아님 |
| 상업용 재설계 73차 데스크톱 홈 | `docs/visual-evidence/current/desktop_1440_home.png` | 1440px 홈에서 좌측 이슈 피드가 지도보다 먼저 읽히고, 우측 지도는 자료 위치·인증 범위 맥락으로 유지. 카드 내부 nested 위치 박스는 평면 행으로 낮아지고 primary는 상세 근거 탭으로 연결된다. API live 동기화와 사용자 수락 전 S+ 완료 증거는 아님 |
