...
| 항목 | 현재 등급 | S+ 기준 | 상태 |
|---|---:|---|---|
| 목적 인지 | A 후보 | 첫 5초 안에 “집회·시위 객관 확인 앱”과 “어떤 구체 이슈가 확인됐나”가 바로 보인다. | Active · 로컬/production fallback은 `정보통신망법 개정 반대 집회`로 시작한다. 현재 live visual은 배포/API 미연결 상태라 첫 이슈가 아직 `정보통신망법 개정 관련 집회`이고 `serviceSyncState=delayed`다. 사용자 수락 전 S+ 아님 |
...
| 상업용 재설계 67차 라이브 visual smoke | `pnpm check:visual-surface:live` | 실제 `https://musunil.com` 390/430/768/1440px에서 홈·상세·영상·탐색·제보 20개 상태 통과. 최신 첫 이슈 `정보통신망법 개정 관련 집회`, topic issue 3개, story 3개, empty state false, source bundle first 0/4. 단 `serviceSyncState=delayed`라 live API 동기화 전 S+ 아님 |
