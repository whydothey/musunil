...
`pnpm sources:assemblies`는 외부 원천 fetch/parse까지만 검증하고 API에 쓰지 않는다. 게시판 원문 parser가 0건이면 구조 변경으로 보고 실패하며, 원문은 정상 파싱됐지만 최근 7일·예정 일정이 없으면 정상 `empty`로 기록한다. `pnpm sources:assemblies:post`는 원천별 결과를 운영 API `/internal/ingest/public-occurrences/batch`에 독립적으로 전송하므로 한 지역의 장애가 다른 지역 저장을 막지 않는다. `pnpm sources:refresh-preflight`는 운영 API의 기존 `sourceRefreshes`를 먼저 확인하고, 부족하며 `MUSUNIL_INTERNAL_API_KEY`가 있으면 `sources:assemblies:post`를 자동 실행한 뒤 다시 확인한다.

운영 API의 `/public-sources/coverage`는 정적 registry의 `lastCheckedAt`만 보지 않는다. 공개 원천 worker가 원천별 점검 시각, 최근 결과 건수, 원문 파싱 건수, `success|empty|partial|failed` 상태를 보내고 API가 이를 `sourceRefreshes`로 저장한다. `lastCheckedAt`은 최근 시도, `lastSuccessfulAt`과 freshness는 최근 성공 또는 정상 empty를 기준으로 계산한다. 따라서 `18/18 parser 준비`는 구조 준비 증거이고, `sourceRefreshes`와 freshness는 실제 scheduler 성공 증거다. 출시 전 `pnpm sources:refresh-preflight`와 `pnpm launch:post-deploy-smoke -- --require-laws --require-source-refreshes`는 18개 활성 일정 원천 모두의 refresh ledger를 요구한다.
