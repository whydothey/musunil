...
법안 관련 뉴스 자동 수집은 별도 키가 필요 없는 국내 언론사 공식 RSS를 사용한다.

- `public_data_sources.news_initial_lookback_days`
- `public_data_sources.news_max_results_per_feed`
- `public_data_sources.news_max_feeds_per_run`
- `public_data_sources.news_min_request_interval_ms`

기본값은 최근 30일, 피드당 100건, 실행당 7개 공식 피드, 호출 간격 0.5초다. 법안명과 핵심 논점 관련성을 로컬에서 판정해 비공개 뉴스 후보로 적재하며, 월 20,000회 내부 안전 한도에서 자동 중단한다.
