...
- UX·디자인은 사용자 판정과 충돌한 문서상 S+ 선언을 리셋했고, 상업용 앱 수준 재설계가 Active다. 1~32차 개선으로 홈 KPI/숫자판/장식 커버/운영자 언어/지도 숫자판/제보 기술 문구/상세 탭 오표기/모바일 카드 과밀/제보 단계표를 낮췄고, 공개 poster 프리뷰·근거 탭 직행·선택 이슈 문맥 라인·지도-first·컴팩트 지도 시트·하단 근거 도구막대·검토 대기 영상 슬롯·지도 중심 데스크톱 레이아웃·dev config 회귀 방어·홈 compact 검토 행·홈 scanline·데스크톱 홈 지역 현황 지도·기본 홈 상세 닫힘·지도 선택 시트 경량화·지도 도구막대/탐색 타일 경량화·공식 fallback 이슈·지도 선택 현장 상세 동기화·모바일 5탭 overflow 방지·홈 주행동 단일화·`인증영상/현장촬영` 라벨 분리·`위치/현장/공식 자료/인원` scanline·지도 fallback 표면 개선·MapLibre layer 재동기화·현장 인증 Evidence 기반 지도 영역 분리·인증 영역 근접 줌을 연결했다. 32차로 공식 자료 핀과 GPS 인증 영역의 오해 가능성은 1차 완화됐지만, 실제 운영 GPS evidence, 실제 제보 영상 품질, 독립 재검증, 사용자 수락 전에는 S+가 아니다.
...
| 2 | 상업용 UI/UX 재설계 | Active | 32차로 지도 인증 영역이 공개 가능한 live Evidence 기준으로만 생성되게 개선했다. 홈 CTA 단일화, `인증영상/현장촬영` 라벨 분리, 지도 표면 개선, 공식 자료 핀과 GPS 인증 영역 문구·줌 분리까지 1차 반영됐지만 실제 운영 GPS evidence, 실제 제보 영상 품질, 독립 재검증, 사용자 수락은 남아 있다. | 32차 캡처를 기준으로 실제 운영 GPS evidence가 붙은 지도 영역과 실제 공개 영상 표면을 재검증한다. | `docs/commercial-splus-redesign.md`, 32차 map 캡처, 30차 home/reels 캡처, web smoke, 사용자 수락 |
...
| 17 | 데스크톱·모바일 디자인 | Active | 문맥 라인, 릴스 겹침, 어두운 영상 poster, 데스크톱 제보 빈 공간, 지도-first, 영상 소셜 레일, 샘플 poster 표시, 홈 대형 검토 썸네일, scanline 혼선, 데스크톱 홈 빈 공간, 패널 경쟁, 지도 시트 과밀, 지도 도구 밀도, 모바일 5탭 overflow, CTA 분산, 영상/촬영 라벨 혼선, 지도 fallback placeholder감, 지도 영역 오해 가능성은 1차 개선됐지만 사용자 수락 전이다. | 32차 캡처로 지도 화면과 모바일 홈이 상업용 앱처럼 읽히는지 재평가하고, 실제 제보 영상 표면을 낮춘다. | `docs/commercial-splus-redesign.md`, 32차 screenshots, `rejectedTerms=[]`, `overflowX=false`, 사용자 수락 |
