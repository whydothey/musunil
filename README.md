...
Render 배포가 끝난 뒤 로컬 셸 없이 검증하려면 GitHub Actions `post-deploy` 수동 workflow를 실행한다. `web-deploy` 모드는 strict Web header와 static hash를 확인하고, `final-gate` 모드는 `pnpm launch:final-gate` 전체를 실행한다. 이 workflow는 push에 자동 실행되지 않으므로 외부 DNS/API 연결 전 일반 커밋을 막지 않는다.
