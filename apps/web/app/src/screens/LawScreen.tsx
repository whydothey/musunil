...
export function LawScreen({ id }: { id: string }) {
  ...
  const topic = dataset?.lawTopics.find((item) => item.id === law?.primaryLawTopicId);
  ...
  return (
    <section className="screen screen-detail law-detail" data-screen="law">
      ...
      {topic ? <section className="content-section law-topic-link-section"><div className="section-heading"><div><h2>대표 주제</h2><p>공식 제안이유·주요내용의 대표 키워드로 묶었습니다</p></div><Layers3 aria-hidden="true" /></div><Link href={`/laws/topics/${encodeURIComponent(topic.id)}`} className="law-related-row"><span><strong>{topic.label}</strong><small>관련 법안 {topic.billCount}건 · {topic.representativeKeywords.join(" · ")}</small></span><Layers3 aria-hidden="true" /></Link></section> : null}
