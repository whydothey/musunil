...
| 2026-07-19 18:55 KST | G7 | 코드·CI는 최신이어도 실제 도메인에 운영 API·보안 헤더·빌드 식별이 없으면 출시 상태로 볼 수 없음 | 라이브 service watch를 재실행해 정적 manifest와 공개 config의 일치는 유지하면서도 build-info placeholder, Web header 누락, API DNS 미연결, non-live fallback 상태를 그대로 blocker로 확정했다. | `service:watch -- --once --with-visual` 실패는 예상된 외부 운영 blocker만 반환. 최신 CI [#29682351283](https://github.com/whydothey/musunil/actions/runs/29682351283) 1m 55s 성공 |
...
| G7 | live + CI | `service:watch -- --once --with-visual` at 2026-07-19T09:55:38.949Z: 정적 manifest 16개/2,458,357 bytes와 local hash 일치, Web config의 공개 필드 2개와 `https://api.musunil.com` 일치, 4 viewport 정직한 fallback surface. [GitHub Actions #29682351283](https://github.com/whydothey/musunil/actions/runs/29682351283) 1m 55s 성공 | **Active / external blocker** | `build-info.json`이 placeholder, `/`·`config.js`·`build-info.json` 보안 헤더 누락, `api.musunil.com` DNS 미연결, Render/Cloudflare token·API target·runtime Secret File·원천/PortOne/storage/비식별/무결성 credential 미입력 |
