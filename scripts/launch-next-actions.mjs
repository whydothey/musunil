...
function parseReport(source, refreshMetadata = { attempted: false }) {
  ...
  return {
    ...
    helperCommands: [
      ...
      "pnpm check:web-render-build-command",
...
function prerequisiteForStage(stage, actions, launchApplyPlan = null, headerApplyPlan = null) {
  ...
  if (stage === "publish_build_metadata") {
    return "`pnpm check:web-render-build-command`로 Render 전용 build contract를 먼저 검증한다. 이후 Render musunil-web Build Command가 pnpm build:web-static:render인지 확인하고 Clear build cache & deploy로 새 산출물을 publish한다.";
  }
