...
- 핵심 약점: 규모 추정은 보수적 자동 산출과 품질 신호 반영 단계까지 왔지만 운영형 영상 분석·실제 모바일 attestation provider dry-run·실제 `pnpm storage:smoke`·`pnpm redaction:smoke`·`pnpm identity:smoke` 실행은 아직 남아 있다.
...
| 실시간 현장 인증 | A+ | 앱 내 촬영, GPS, 거리, 업로드 시간, 서버 해시, 기기 무결성, 비식별 공개본이 한 흐름으로 검증된다. | 브라우저 흐름 있음 · 검수 전 held_private · Proof-of-Presence는 `in_app_camera`와 최소 5초만 인정 · 5분 제한은 서버 기록 시각 기준 · 현장 판단 Claim도 verifier 전 held_private · 공개 전환은 내부 verifier device integrity `pass`와 proof hash 필요 · 비공개 R2 PUT/GET/DELETE와 내장 영상 비식별 smoke 통과 · 내부 queue/worker와 비공개 파생본 proxy 구현 · 실제 배포 worker/모바일 attestation/PortOne end-to-end smoke 실행 남음 |
...
| 개인정보/권리 보호 | A+ | 원본 비공개, 비식별 공개본, 위치 흐림, 권리 검토 상태가 기본이다. | private path·정밀 위치값 비노출 dashboard 완료 · LIVE 공개 응답은 `publicRadiusM`만 노출 · 외부 R2 read-back과 실제 영상 redaction smoke 통과 · 배포 worker/purge 검증 필요 |
