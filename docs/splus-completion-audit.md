...
| 실시간 현장 인증 | A+ Active | 앱 내 촬영, 최소 5초, 서버 시각, held_private, redaction proof, trusted device integrity gate, 공개 poster 필수, 내부 비식별 queue/worker, 비공개 R2 PUT/GET/DELETE, 실제 영상 redaction smoke | 배포 worker end-to-end, Play Integrity/App Attest dry-run |
...
| 개인정보/권리 보호 | A+ Active | private key/raw GPS 비노출, 공개 반경, poster 없는 clip 공개 차단, purge 전 외부 delete gate, 버킷 단위 최소 권한 R2 credential과 공개 접근 차단, 실제 비식별 smoke | 배포 worker와 purge end-to-end 검증 |
