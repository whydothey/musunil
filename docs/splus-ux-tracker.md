...
Last updated: 2026-07-12 15:22 KST
...
| 첫 화면 구조 | A+ 후보 | 평균 사용자가 첫 화면에서 주 행동과 상세 진입을 설명 없이 이해한다. | Active · 주요 이슈 story ring, 카드 action 1개 `상세 보기`, 반복 감사 라벨 제거, 상단 숫자판 DOM 제거, 390/430/768/1440px 실제 렌더링 `overflowX=false` |
...
| 화면 | 파일 | 확인 |
|---|---|---|
| 상업용 재설계 72차 현재 evidence 세트 | `docs/visual-evidence/current/visual-surface-evidence.json` | `pnpm check:visual-surface:evidence`가 390/430/768/1440px의 홈·상세·영상·탐색·제보 20개 PNG를 생성. 현재 로컬 첫 이슈는 `정보통신망법 개정 관련 집회`, `sourceBundleFirst=false`, forbidden 0, `serviceSyncState=delayed`, 배너 `공개자료로 먼저 확인`. 홈·상세·지도 요약은 `자료 기준`/`일시 확인 중` 조합 대신 장소와 사용자용 기준일, 공식자료·현장영상·반론/정정 흐름으로 표시됨 |
| 상업용 재설계 72차 모바일 홈 | `docs/visual-evidence/current/mobile_390_home.png` | 390px 홈에서 story ring, 이슈 카드 2개, 하단 5탭, 배너 `공개자료로 먼저 확인`, 첫 카드 `서울 도심권 · 수도권 · 2026.07.12 기준`, `상세 보기/다른 주장` 경로 유지. API live 동기화 전이라 S+ 완료 증거는 아님 |
| 상업용 재설계 72차 데스크톱 홈 | `docs/visual-evidence/current/desktop_1440_home.png` | 1440px 홈에서 좌측 이슈 피드가 지도보다 먼저 읽히고, 우측 지도는 자료 위치·인증 범위 맥락으로 유지. 카드·상세·지도 요약은 `자료 기준` 대신 사용자용 기준일과 근거 흐름으로 압축된다. API live 동기화와 사용자 수락 전 S+ 완료 증거는 아님 |
