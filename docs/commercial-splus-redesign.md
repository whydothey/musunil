...
- 2026-07-12 13:46 패치로 Web header blocker 전용 경로 `pnpm launch:apply -- --cloudflare-headers-only`를 추가했다. 이 경로는 Render target/API DNS 없이 `CLOUDFLARE_API_TOKEN`만 요구하며, 운영 브리프와 service watch가 Web header 미적용 상태에서 이 명령을 우선 fallback으로 안내한다. 실제 Cloudflare 적용과 strict header 검증 전에는 출시 S+가 아니다.
