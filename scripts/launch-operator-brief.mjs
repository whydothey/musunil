...
function renderMarkdown(value) {
  return [
    ...
    "## Render API Service",
    "",
    `- Blueprint Path: \`${value.renderApiService.blueprintPath || "render.backend.yaml"}\``,
    "- 기존 `musunil-web`은 수동 관리 상태로 보존하며 이 Blueprint에 포함하지 않는다.",
    `- 예상 최소 비용: 월 USD ${value.renderApiService.estimatedMinimumUsdPerMonth || 14} + 사용량 + 세금`,
    "- 배포 전 확인: `pnpm render:provisioning-plan`",
    ...
    "Runtime Secret File:",
    "",
    "- `pnpm render:runtime-secret`은 로컬 YAML, 파일 권한, 대상 서비스 존재 여부를 확인하는 dry-run이다.",
    "- 실제 적용은 `RENDER_API_TOKEN=... MUSUNIL_RENDER_SECRET_APPLY_CONFIRM=APPLY_RUNTIME_SECRET_FILE pnpm render:runtime-secret -- --apply`로만 한다.",
    "- 같은 `musunil.user-inputs.yaml`을 `musunil-api`와 `musunil-ops-scheduler`에 업로드하고 두 서비스 모두 `/etc/secrets/musunil.user-inputs.yaml`을 읽는다.",
    "- Secret File 내용과 credential 값은 명령 출력에 표시하지 않는다.",
    "",
    "Render API automation:",
    "",
    ...
    "- custom domain/header 자동화는 Secret File을 교체하지 않는다. Secret File은 위의 별도 확인 절차로만 적용한다.",
    "",
    "## Cloudflare",
    "",
