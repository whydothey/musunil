...
Last updated: 2026-07-12 21:37 KST
...
| 항목 | 현재 등급 | S+ 기준 | 상태 |
|---|---:|---|---|
| 목적 인지 | A 후보 | 첫 5초 안에 “집회·시위 객관 확인 앱”과 “어떤 구체 이슈가 확인됐나”가 바로 보인다. | Active · 로컬/preview와 live fallback 모두 구체 이슈로 시작한다. 최신 live visual 첫 이슈는 `정보통신망법 개정 관련 집회`지만 API 미연결으로 `serviceSyncState=delayed`다. 사용자 수락 전 S+ 아님 |
| 첫 화면 구조 | A+ 후보 | 평균 사용자가 첫 화면에서 주 행동과 상세 진입을 설명 없이 이해한다. | Active · 주요 이슈 story ring, 카드 primary `근거·영상`, 평면 `지도에서 보기` 행, 반복 감사 라벨 제거, 상단 숫자판 DOM 제거, 모바일 API 지연 상태줄 30px 이하, 390/430/768/1440px 실제 렌더링 `overflowX=false` |
| 이슈 중심 흐름 | A+ 후보 | 이슈 선택 → 상세/근거/지도/영상/반론이 같은 이슈 맥락으로 이어진다. | Active · 홈 주행동은 `근거·영상`, 위치 행은 `지도에서 보기`, 반론 있는 이슈는 `반론·정정`; primary는 상세 근거 탭으로 진입 |
...
| 모바일 UX | A+ 후보 | 390px 기준 가로 넘침 없이 이슈 2개, 위치·근거 문장, 핵심 행동이 보인다. 실제 공개 영상이 없으면 가짜 썸네일을 만들지 않는다. | Active · 390px 하단 탭 `홈/영상/탐색/법안/제보`, `overflowX=false`, 홈 지도 미노출, 위치/근거 요약과 `근거·영상` primary |
...
| 화면 | 파일 | 확인 |
|---|---|---|
| 상업용 재설계 75차 현재 evidence 세트 | `docs/visual-evidence/current/visual-surface-evidence.json` | `pnpm check:visual-surface:evidence`가 390/430/768/1440px의 홈·상세·영상·탐색·제보 20개 PNG를 생성. 현재 로컬 첫 이슈는 `정보통신망법 개정 관련 집회`, `sourceBundleFirst=false`, forbidden 0, `serviceSyncState=delayed`, 배너 `공개자료로 먼저 확인`. 모바일 홈은 상태줄 높이 30px 이하와 중복 새로고침 버튼 미노출까지 검사한다 |
| 상업용 재설계 75차 모바일 홈 | `docs/visual-evidence/current/mobile_390_home.png` | 390px 홈에서 story ring, 이슈 카드 2개, 하단 5탭, 버튼 없는 얇은 `공개자료로 먼저 확인` 상태줄, 첫 카드 `서울 도심권 · 수도권 · 2026.07.12 기준`, `근거·영상/반론·정정` 경로 유지. API live 동기화 전이라 S+ 완료 증거는 아님 |
