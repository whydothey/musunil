...
현재 사용자 경험 등급은 A 후보 범위다. 상업용 S+ 재설계는 Active다. 2026-07-11 06:11 KST 독립 재검토는 최신 홈을 Visual C+~A- 후보, 국평오 5초 PASS-/10초 PARTIAL/20초 PARTIAL로 봤다. 26차 개선은 데스크톱 기본 홈의 우측 상세 패널을 닫아 첫 화면을 `이슈 목록 + 지역 현황 지도` 2축으로 단순화했고, 27차 개선은 지도 선택 시트를 데스크톱 62px/모바일 114px까지 낮췄다. 사용자 수락 전까지 S+로 표기하지 않는다.

- 사용 가능성: 홈에서 이슈명, 어디서 확인됐는지, 근거 진입이 보이고, 데스크톱 상세에서 영상·지도·근거로 바로 이동할 수 있다.
- 디자인 완성도: A-~A+ 후보
- 국평오 직관성: B-~A+ 후보
- 핵심 문제: 데스크톱 패널 경쟁과 지도 시트 과밀은 1차 완화됐지만, 탐색 타일/지도 도구 밀도, 실제 제보 영상 품질, 사용자 수락은 아직 S+가 아니다.
...
| 항목 | 현재 등급 | S+ 기준 | 상태 |
|---|---:|---|---|
| 목적 인지 | A+ | 첫 5초 안에 “집회·시위 객관 확인 앱”과 “어떤 공개자료가 확인됐나”가 바로 보인다. | 개선 중 · 홈 제목 `집회·시위 공개자료`, 운영자 언어 노출 0 |
| 첫 화면 구조 | A 후보 | 평균 사용자가 첫 화면에서 주 행동과 상세 진입을 설명 없이 이해한다. | Active · 데스크톱 기본 홈은 상세 패널 없이 이슈+지도 2축으로 시작. 390px/1440px `overflowX=false` |
| 이슈 중심 흐름 | A 후보 | 이슈 선택 → 근거/영상/지역/반론이 같은 이슈 맥락으로 이어진다. | Active · `근거/반론`은 상세 패널, `영상`은 영상 탭, `지역`은 지도 탭으로 검증됨 |
| 상황 상세 | A+ 후보 | 무슨 일, 장소·시간, 확인 근거, 아직 확인할 점이 숫자판보다 먼저 보인다. | Active · 상세 상단 `labelDisplay=none`, 3문장 요약, 빠른 버튼 `근거/영상/지도`, overflow 없음 |
| 현장 영상 UX | A 후보 | 영상이 확정 결론이 아니라 현장 인증 자료이며, 근거·위치 범위·관련 이슈에 접근한다. | Active · 샘플 poster는 실제 영상처럼 표시하지 않고 검토 대기 슬롯으로 렌더. 모바일 `posterImages=0`, `reviewSlots=3`, 금지 문구 0. 실제 제보 영상 품질과 사용자 수락 필요 |
| 제보 UX | A 후보 | 첫 제보자가 위치 확인, 대상 확정, 촬영, 미리보기, 접수 상태를 한 흐름으로 이해한다. | Active · 탭 라벨 `영상제보`, 첫 CTA `근처 현장 찾기`, GPS 전 단계표/칩/후보/대상/미리보기 숨김. 독립 검증 필요 |
| 지도 UX | A+ 후보 | 지도는 자료 위치와 현장 인증 범위만 보여주고 이슈 파일과 연결된다. | Active · 데스크톱 홈 지도 sheet 62px, 모바일 지도 sheet 114px, detail hidden, 지도 탭은 우측 인스펙터 없이 map width 1200px. 모바일 홈은 지도 미노출 |
| 모바일 UX | A+ 후보 | 390px 기준 가로 넘침 없이 이슈 2개, 위치·근거 문장, 핵심 행동이 보인다. 실제 공개 영상이 없으면 가짜 썸네일을 만들지 않는다. | Active · 390/430/768px `overflowX=false`, scanline clipped false, 홈 지도 미노출, visible issue cards 2, `영상제보` 라벨 |
