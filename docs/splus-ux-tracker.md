...
2026-07-20 최신 판정은 **UI 출시 후보 Guard**다. 기존 1~76차 단일 HTML 개선 이력은 React/Vite 전면 재구축으로 대체됐다. 홈은 단일 주요 이슈 피드, 상세는 전체 화면 `현장/영상/근거/법안`, 탐색은 실제 MapLibre 지도, 영상은 세로 snap, 법안은 두 정렬과 상세, 제보는 7단계 한 행동 흐름으로 재구성했다. 독립 QA의 회색 지도·릴스 근거 이동·법안 상세·제보 단계 검증·데스크톱 폭 지적을 모두 수정했고, fixture 68개 캡처와 실제 `musunil.com` 390/430/768/1440 검증이 통과했다. production은 API 미연결 시 `serviceSyncState=unavailable`로 두고 가상 이슈·영상·법안을 표시하지 않는다. 따라서 UI 자체는 Guard지만 실제 운영 데이터가 있는 5·10·20초 흐름과 현장 GPS 영역·공개 영상 품질은 G7 전까지 전체 S+ 증거가 아니다.

현재 사용자 경험 등급은 A 후보 범위다. 상업용 S+ 재설계는 Active다. 2026-07-11 06:11 KST 독립 재검토는 최신 홈을 Visual C+~A- 후보, 국평오 5초 PASS-/10초 PARTIAL/20초 PARTIAL로 봤다. 26차 개선은 데스크톱 기본 홈의 우측 상세 패널을 닫아 첫 화면을 `이슈 목록 + 지역 현황 지도` 2축으로 단순화했고, 27~32차 개선은 지도 선택 시트·검색·범례·탐색 타일·fallback 표면·GPS evidence 기반 영역 생성을 낮췄다. 33차 개선은 poster 없는 인증영상을 빈 영상판이 아니라 검토 카드로 낮췄고, 34~35차는 실제 공개 영상이 붙으면 풀스크린 video player로 열리고 미디어 route/CSP가 동작하게 했다. 50차는 데스크톱 홈 지도 과점을 줄였고, 51차는 poster 없는 영상을 홈 카드 썸네일처럼 보이지 않게 했으며, 53차는 주요 이슈 레일을 필터칩이 아닌 원형 이슈 story ring으로 바꿨다. 2026-07-12 01:29 KST에는 숨겨진 상단 숫자판 DOM과 갱신 로직을 제거하고, `pnpm check:visual-surface`를 추가해 390/430/768/1440px 실제 브라우저 렌더링에서 홈·상세·영상·탐색·제보 흐름을 검사한다. 08:18 KST에는 첫 이슈 카드가 다시 대시보드처럼 변하지 않도록 카드 액션 1~2개, primary action 1개, interactive target 3개 이하, 카드 내부 chip 0개, 카드 높이 상한을 visual gate에 추가했다. 10:08 KST live visual은 실제 `https://musunil.com`에서 390/430/768/1440px 화면 구조를 통과했고 첫 이슈는 `정보통신망법 개정 관련 집회`, 홈 topic issue 3개, story 3개, source bundle first 0/4다. 14:50 KST에는 `pnpm check:visual-surface:evidence`가 현재 로컬 20개 화면 PNG와 JSON을 `docs/visual-evidence/current`에 남기게 했고, 홈 카드의 표형 `위치/근거` 라벨을 화면에서 숨겨 이슈 카드가 대시보드 row처럼 보이는 신호를 줄였다. 14:57 KST에는 API 연결 지연 배너를 `저장된 공개자료 기준`에서 `공개자료로 먼저 확인`으로 바꿔 내부 캐시성 표현을 낮췄다. 15:08 KST에는 홈 카드 첫 줄의 `자료 기준` 표현을 `2026.07.12 기준`처럼 사용자용 기준일로 낮추고, 카드 요약을 `현장·위치`와 `공식자료·현장영상·반론/정정` 두 흐름으로 압축했다. 15:22 KST에는 상세와 지도 선택 요약까지 `일시 확인 중 · 자료 기준` 조합을 제거하고, 공개 화면에서는 `서울 도심권 · 수도권 · 2026.07.12 기준`처럼 장소와 기준일만 먼저 보이게 통일했다. 다만 Web `serviceSyncState=delayed`라 운영 API live 동기화는 아직 미달이며, `pnpm service:watch:visual`은 이 상태를 실패로 유지한다. 운영 API가 live topic Issue를 반환하고 사용자 수락을 받기 전까지 UX를 S+로 표기하지 않는다.

2026-07-12 19:43 KST 독립 Visual Critique와 IA Red-Team은 홈이 여전히 KPI 문자열, nested 위치 박스, 생성형 story visual, 모호한 `상세 보기` 때문에 상업용 앱보다 대시보드처럼 읽힌다고 평가했다. 최신 패치는 홈 제목을 `공개자료 기반 집회·시위`로 낮추고, story ring fallback을 추상 상태 마크로 바꾸며, nested 위치 박스를 평면 `지도에서 보기` 행으로 낮추고, primary CTA를 `근거·영상`으로 바꿔 상세의 근거 탭에 바로 진입하게 했다. `pnpm check:web-smoke`, `pnpm check:ux-surface`, `pnpm check:web-flow`, `pnpm check:visual-surface:evidence`는 통과했지만 실제 live API 동기화와 사용자 수락 전에는 S+가 아니다.

2026-07-12 19:53 KST에는 poster 없는 LIVE Claim을 더 이상 `reel-full` 영상판처럼 렌더하지 않게 바꿨다. 실제 공개 clip과 poster가 모두 있을 때만 `<video class="reel-video">` 풀스크린 플레이어를 쓰고, 공개 전 자료는 `reel-pending-card`로 낮춰 `현장 영상 공개 준비 중`, 위치 반경, 길이, 상태, 근거/위치/반론/이슈 액션만 보여준다. `pnpm check:web-smoke`와 `pnpm check:visual-surface:evidence`가 poster 없는 자료가 풀스크린 영상처럼 보이지 않는지 검사한다. 그래도 실제 운영 공개 영상 캡처와 사용자 수락 전에는 S+가 아니다.

2026-07-12 21:37 KST에는 API 연결 지연 상태가 모바일에서 별도 조작 영역처럼 보이지 않도록 상단 `service-banner`를 버튼 없는 얇은 출처 상태줄로 낮췄다. 모바일 홈 visual gate는 상태줄 높이 30px 이하와 중복 새로고침 버튼 미노출을 검사한다. `pnpm check:visual-surface:evidence`는 390/430/768/1440px 20개 화면 PNG/JSON을 다시 생성했고, 실제 live API 동기화와 사용자 수락 전에는 계속 S+로 표기하지 않는다.

2026-07-12 22:35 KST에는 독립 비평의 P0를 다시 반영해 홈과 영상의 약속 범위를 정리했다. 로컬/production fallback 첫 이슈는 넓은 관련 쟁점보다 구체 주제인 `정보통신망법 개정 반대 집회`가 먼저 보이고, 카드·지도 시트 주행동은 `근거 보기`로 고정됐다. 실제 공개 clip+poster가 모두 있는 이슈에서만 `영상 보기`가 보이므로, 검토 중 촬영자료를 공개 영상처럼 약속하지 않는다. 주요 이슈 레일은 poster 없는 경우 생성형 원형 비주얼 대신 텍스트형 이슈 칩으로 낮췄고, 제보 첫 화면은 위치 권한 전에도 후보 선택에서 확인할 정보 3가지를 보여준다. `pnpm check:web-smoke`, `pnpm check:web-flow`, `pnpm check:ux-surface`, `pnpm check:visual-surface:evidence`가 이 계약을 검사한다. 실제 live API 동기화와 사용자 수락 전에는 계속 S+로 표기하지 않는다.

2026-07-12 22:51 KST에는 실제 `https://musunil.com`을 대상으로 `pnpm check:visual-surface:live:evidence`를 실행해 390/430/768/1440px 홈·상세·영상·탐색·제보 20개 운영 화면 PNG와 JSON을 `docs/visual-evidence/live-current`에 남겼다. 모든 시각 상태는 통과했고 live 첫 이슈도 `정보통신망법 개정 반대 집회`로 확인됐으며 `sourceBundleFirst=false`다. 다만 전체 상태가 `serviceSyncState=delayed`이고 서비스 상태줄이 20개 화면에 노출되므로, 이는 최신 UI 배포 증거이지 운영 API live 동기화 완료 증거가 아니다.
...
| 2 | IA 3안 발산 | 이슈 파일 First, 내 주변 Map/List First, 공공 조회센터를 390px/1440px 흐름으로 비교한다. | 완료 · 단일 이슈 피드/화면 전환형 상세 선택 |
| 3 | 레퍼런스 기반 선택 | Citizen, PulsePoint, Naver/KakaoMap, Toss, GOV.UK 기준으로 선택된 IA만 구현한다. | 완료 · 독립 QA 반영 |
| 4 | 시각 언어 재설계 | KPI 타일, 카드 내 숫자판, 장식성 글로우, 과도한 칩을 제거한다. | 1차 완료 |
| 5 | 사용 흐름 구현 | 이슈 파악 → 상세 → 근거 → 지도 → 현장 영상 흐름을 설명 없이 이어지게 한다. | 2차 완료 |
| 6 | 상업용 S+ 검증 | 390/430/768/1440px 캡처와 실제 라이브 증거를 통과한다. | Guard · React fixture 68개 캡처와 live-current 검증 통과, 실제 운영 데이터 흐름은 G7 |

## Scorecard

| 항목 | 현재 등급 | S+ 기준 | 상태 |
|---|---:|---|---|
| 목적 인지 | A 후보 | 첫 5초 안에 “집회·시위 객관 확인 앱”과 “어떤 구체 이슈가 확인됐나”가 바로 보인다. | Active · 로컬/production fallback과 최신 live visual 모두 `정보통신망법 개정 반대 집회`로 시작한다. 다만 live는 API 미연결 상태라 `serviceSyncState=delayed`이고 사용자 수락 전 S+ 아님 |
| 첫 화면 구조 | A+ 후보 | 평균 사용자가 첫 화면에서 주 행동과 상세 진입을 설명 없이 이해한다. | Active · 주요 이슈 레일, 카드 primary `근거 보기`, 공개본 있을 때만 `영상 보기`, 평면 `지도에서 보기` 행, 반복 감사 라벨 제거, 상단 숫자판 DOM 제거, 모바일 API 지연 상태줄 30px 이하, 390/430/768/1440px 실제 렌더링 `overflowX=false` |
| 이슈 중심 흐름 | A+ 후보 | 이슈 선택 → 상세/근거/지도/영상/반론이 같은 이슈 맥락으로 이어진다. | Active · 홈 주행동은 `근거 보기`, 위치 행은 `지도에서 보기`, 공개 가능한 영상이 있는 이슈만 `영상 보기`, 반론 있는 이슈는 `반론·정정`; primary는 상세 근거 탭으로 진입 |
| 상황 상세 | A+ 후보 | 무슨 일, 장소·시간, 확인 근거, 아직 확인할 점이 숫자판보다 먼저 보인다. | Active · 상세 상단 `labelDisplay=none`, 3문장 요약, 빠른 버튼 `근거/인증 영상/지도`, overflow 없음 |
| 현장 영상 UX | A+ 후보 | 영상이 확정 결론이 아니라 현장 인증 자료이며, 근거·위치 범위·관련 이슈에 접근한다. | Active · 실제 공개 clip+poster가 모두 있으면 풀스크린 `<video class="reel-video">`로 표시하고, poster 없는 LIVE Claim은 `reel-pending-card`로 낮춰 재생 가능한 영상처럼 보이지 않게 한다. media route/CSP와 pending-fullscreen 회귀 방지 gate가 통과한다. 실제 운영 공개 영상 캡처와 사용자 수락 필요 |
...
| 모바일 UX | A+ 후보 | 390px 기준 가로 넘침 없이 이슈 2개, 위치·근거 문장, 핵심 행동이 보인다. 실제 공개 영상이 없으면 가짜 썸네일을 만들지 않는다. | Active · 390px 하단 탭 `홈/영상/탐색/법안/제보`, `overflowX=false`, 홈 지도 미노출, 위치/근거 요약과 `근거 보기` primary |
...
| 화면 | 파일 | 확인 |
|---|---|---|
| 상업용 재설계 live-current 운영 evidence 세트 | `docs/visual-evidence/live-current/visual-surface-evidence.json` | `pnpm check:visual-surface:live:evidence`가 실제 `https://musunil.com`에서 390/430/768/1440px의 홈·상세·영상·탐색·제보 20개 PNG를 생성. 모든 상태는 통과했고 live 첫 이슈는 `정보통신망법 개정 반대 집회`, `sourceBundleFirst=false`, 배너 `공개자료로 먼저 확인`. 단 `serviceSyncState=delayed`라 운영 API live 동기화 완료 증거는 아님 |
| 상업용 재설계 76차 현재 evidence 세트 | `docs/visual-evidence/current/visual-surface-evidence.json` | `pnpm check:visual-surface:evidence`가 390/430/768/1440px의 홈·상세·영상·탐색·제보 20개 PNG를 생성. 현재 로컬 첫 이슈는 `정보통신망법 개정 반대 집회`, `sourceBundleFirst=false`, forbidden 0, `serviceSyncState=delayed`, 배너 `공개자료로 먼저 확인`. 모바일 홈은 상태줄 높이 30px 이하, 중복 새로고침 버튼 미노출, 공개 영상과 검토 중 촬영자료 분리를 검사한다 |
| 상업용 재설계 76차 모바일 홈 | `docs/visual-evidence/current/mobile_390_home.png` | 390px 홈에서 텍스트형 주요 이슈 레일, 이슈 카드 2개, 하단 5탭, 버튼 없는 얇은 `공개자료로 먼저 확인` 상태줄, 첫 카드 `서울 · 현장 2건 · 공개 위치 1곳`, `근거 보기/반론·정정` 경로 유지. API live 동기화 전이라 S+ 완료 증거는 아님 |
| 상업용 재설계 74차 모바일 영상 | `docs/visual-evidence/current/mobile_390_reels.png` | 390px 영상 탭에서 `현장 영상 공개 준비 중` 검토 카드, 위치 반경, 길이, 상태, `근거/위치/반론/이슈` 액션이 보인다. 재생 가능한 공개본처럼 보이는 어두운 fake video surface는 제거됐지만 실제 운영 공개 영상 캡처 전 S+ 완료 증거는 아님 |
| 상업용 재설계 74차 데스크톱 홈 | `docs/visual-evidence/current/desktop_1440_home.png` | 1440px 홈에서 좌측 이슈 피드가 지도보다 먼저 읽히고, 우측 지도는 자료 위치·인증 범위 맥락으로 유지. 카드 내부 nested 위치 박스는 평면 행으로 낮아지고 primary는 상세 근거 탭으로 연결된다. API live 동기화와 사용자 수락 전 S+ 완료 증거는 아님 |
| 상업용 재설계 74차 데스크톱 영상 | `docs/visual-evidence/current/desktop_1440_reels.png` | 1440px 영상 탭도 poster 없는 LIVE Claim을 검토 카드로 낮추고 우측 맥락 패널과 같은 이슈를 유지한다. 공개 clip+poster가 붙은 운영 데이터에서는 풀스크린 video player 계약을 사용해야 한다 |
...
| 상업용 재설계 55차 모바일 홈 | `docs/commercial-splus-surface55-mobile-390-2026-07-11.png` | 390px 홈 첫 카드가 `서울 · 현장 2건 · 위치 1곳 · 영상 1건 · 인원 추정 검토`를 같은 위치에 표시하고 `상세 보기/다른 주장` 진입을 제공. `overflowX=false`, forbidden 0 |
| 상업용 재설계 55차 모바일 홈 430 | `docs/commercial-splus-surface55-mobile-430-2026-07-11.png` | 430px 홈 첫 카드 206px, 이슈 카드 3개 노출, `overflowX=false`, forbidden 0 |
| 상업용 재설계 55차 태블릿 홈 768 | `docs/commercial-splus-surface55-tablet-768-2026-07-11.png` | 768px 홈 첫 카드 206px, 이슈 카드 3개 노출, `overflowX=false`, forbidden 0 |
| 상업용 재설계 55차 데스크톱 홈 | `docs/commercial-splus-surface55-desktop-1440-2026-07-11.png` | 1440px 홈 이슈 카드 폭이 520px로 고정돼 긴 주제 잘림이 줄고, 지도 맥락은 648x410px로 유지. `overflowX=false`, forbidden 0 |
| 상업용 재설계 56차 모바일 상세 | `docs/commercial-splus-surface56-detail-mobile-390-2026-07-11.png` | 상세 개요를 카드형 리포트에서 `어디서/얼마나/근거/다른 주장/아직 모르는 점` 문장형 답변으로 전환. overview card 0, answer row 5, disclosure 2, `overflowX=false`, forbidden 0 |
| 상업용 재설계 56차 모바일 지도 | `docs/commercial-splus-surface56-map-mobile-390-2026-07-11.png` | 탐색 지도는 자료 위치와 현장 인증 범위, 단일 시트 CTA만 표시. map 370x460, sheet 114px, summary chip 0, `overflowX=false`, forbidden 0 |
| 상업용 재설계 56차 데스크톱 상세 | `docs/commercial-splus-surface56-detail-desktop-1440-2026-07-11.png` | 데스크톱 상세도 answer row 5/disclosure 2로 통일하고, 상세 열린 상태의 지도 시트는 62px compact로 낮춤. `overflowX=false`, forbidden 0 |
| 상업용 재설계 56차 데스크톱 지도 | `docs/commercial-splus-surface56-map-desktop-1440-2026-07-11.png` | 탐색 지도 1200x700, sheet 62px, 지도 선택과 근거·영상 진입만 표시. `overflowX=false`, forbidden 0 |
| 상업용 재설계 68차 모바일 상세 압축 | `docs/commercial-splus-surface68-mobile-detail-compact-390-2026-07-12.png` | 390px 상세 시트에서 장문 설명문을 개요 탭으로 낮추고 상단은 제목·확인 요약·3개 행동만 유지. 실제 브라우저 캡처 `hero=292px`, `actionRow=340px`, `scrollWidth=390`, `detailSummaryVisible=false`, forbidden 0 |
| 상업용 재설계 67차 홈 위치 진입 행 | `pnpm check:visual-surface` | 홈 카드 반복 미니맵을 제거하고 `위치/근거` 2행 요약과 얇은 `지도` 진입 행으로 전환. 390px/430px/768px/1440px CDP 렌더링에서 decorative mini-map/area count 0, 첫 카드 높이 기준 통과 |
| 상업용 재설계 64차 모바일 홈 카드 흐름 | `docs/commercial-splus-surface64-home-card-flow-mobile-390-2026-07-12.png` | 첫 카드 반복 요약을 줄여 `제목 → 지역/현장/위치 → 영상/인원 → 공식/다른 주장` 순서로 표시. 390px `firstCardHeight=206`, `scrollWidth=390` |
| 상업용 재설계 64차 데스크톱 홈 카드 흐름 | `docs/commercial-splus-surface64-home-card-flow-desktop-1440-2026-07-12.png` | 데스크톱도 같은 카드 문장 위계 유지. 1440px `firstCardHeight=216`, `scrollWidth=1440` |
| 상업용 재설계 65차 모바일 홈 맥락 회귀 | `docs/commercial-splus-surface65-home-context-mobile-390-2026-07-12.png` | 모바일 홈은 지도 미노출, 이슈 피드 우선 유지. 390px CDP `mapVisible=false`, `scrollWidth=390`, forbidden 0 |
| 상업용 재설계 65차 데스크톱 홈 지도 맥락화 | `docs/commercial-splus-surface65-home-context-desktop-1440-2026-07-12.png` | 데스크톱 홈의 이슈 피드는 698px, 지도는 380x288px로 축소되어 지도보다 이슈 선택이 먼저 읽힘. 검색 입력은 홈 지도에서 숨기고 탐색 탭의 큰 지도에만 유지. `scrollWidth=1440`, forbidden 0 |
| 상업용 재설계 66차 라이브 모바일 홈 | `docs/commercial-splus-surface66-live-home-mobile-390-2026-07-12.png` | 실제 `https://musunil.com` 390px 캡처. `serviceSyncState=delayed`, 배너 `저장된 공개자료 기준`, 첫 카드 `지역별 집회 공개 일정`, `storyCount=3`, `issueCardCount=3`, `scrollWidth=390`, forbidden 0. 구조는 안정적이나 구체 이슈 우선과 live 데이터 동기화는 미달 |
| 상업용 재설계 66차 라이브 데스크톱 홈 | `docs/commercial-splus-surface66-live-home-desktop-1440-2026-07-12.png` | 실제 `https://musunil.com` 1440px 캡처. 이슈 카드 3개, 지도 380x288px, `serviceSyncState=delayed`, 첫 카드 `지역별 집회 공개 일정`, `scrollWidth=1440`, forbidden 0. live API 연결 전 S+ 아님 |
| 상업용 재설계 67차 라이브 visual smoke | `pnpm check:visual-surface:live` | 실제 `https://musunil.com` 390/430/768/1440px에서 홈·상세·영상·탐색·제보 20개 상태 통과. 최신 첫 이슈 `정보통신망법 개정 반대 집회`, topic issue 3개, story 3개, empty state false, source bundle first 0/4. 단 `serviceSyncState=delayed`라 live API 동기화 전 S+ 아님 |
| 상업용 실제 브라우저 게이트 | `pnpm check:visual-surface` | 390px, 430px, 768px, 1440px에서 홈·상세·영상·탐색·제보 20개 상태가 통과. 대시보드 숫자판 visible 0, forbidden 0, 하단 내비 겹침 0, 지도 시트 과밀 없음. 홈 첫 이슈 카드는 action 1~2개, primary action 1개, interactive target 3개 이하, 내부 chip 0개, 모바일 높이 260px 이하를 검사한다. 결과에는 `serviceStates`와 서비스 배너 상태도 포함 |
...
| 시간 | 항목 | 변경 | 판정 |
|---|---|---|---|
| 2026-07-12 15:22 KST | 상업용 상세·지도 문구 | 상세와 지도 선택 요약에서 `일시 확인 중 · 자료 기준` 조합을 제거하고, 공개 화면은 `장소 · 기준일`과 근거 흐름을 먼저 보게 했다. `pnpm check:visual-surface:evidence`는 상세/지도 요약에 `자료 기준`이 다시 노출되면 실패한다. | Guard 강화 · 실제 live API 동기화와 사용자 수락 전 S+ 아님 |
...
| 2026-07-11 21:26 KST | 운영 배포 확인 | 배포 판정을 build-info 단일 기준에서 static hash까지 함께 보는 구조로 바꿨다. 이후 출시 기준은 다시 강화되어 최신 UI 파일 해시가 맞아도 build metadata placeholder와 header 문제는 blocker로 남긴다. | 운영 확인 안정화 · 사용자 수락 전 S+ 아님 |
...
| 2026-07-11 23:58 KST | 상업용 S+ 재설계 55차 | 독립 Visual/IA 비평을 반영해 API 미연결 배너를 장애성 문구에서 `저장된 공개자료 기준`으로 낮추고, 홈 카드 요약을 `지역 · 현장 · 위치 · 영상 · 인원` 고정 문장으로 바꿨다. 데스크톱 이슈 피드는 1열 520px 카드로 바꿔 제목 잘림을 줄였고, 반론 있는 이슈와 영상에는 `다른 주장/반론` 진입을 추가했다. 390px/1440px 캡처에서 forbidden 0, `overflowX=false`를 확인했다. | A+ 후보 Active · 상세/지도 대시보드화, 실제 운영 영상/GPS, 사용자 수락 전 S+ 아님 |
| 2026-07-12 04:35 KST | API 미연결 빈 피드 UX | live API 또는 `/home.issueCards`가 비어 있을 때 홈이 빈 리스트처럼 보이지 않도록 `새 이슈를 확인 중입니다` controlled empty state와 `다시 확인/지역 보기` 회복 경로를 유지한다. visual smoke는 이슈 0개 실패 detail에 empty state와 action 존재 여부를 함께 기록한다. | Active · API live 연결 전 S+ 아님 |
| 2026-07-11 23:59 KST | 상업용 S+ 재설계 56차 | 상세 개요를 overview card 3개+disclosure 3개에서 `어디서/얼마나/근거/다른 주장/아직 모르는 점` answer row 5개+disclosure 2개로 정리했다. 데스크톱 상세와 지도 경쟁을 줄이기 위해 상세 열린 상태의 지도 시트를 220px에서 62px compact로 낮췄고, 빠른 버튼의 오독되는 아이콘을 숨겼다. 390px/1440px 상세·지도 캡처에서 forbidden 0, `overflowX=false`를 확인했다. | A+ 후보 Active · 실제 운영 공개 영상/GPS, 사용자 수락 전 S+ 아님 |
| 2026-07-12 03:37 KST | live 화면 증거 | 실제 `musunil.com` 모바일/데스크톱 캡처를 추가했다. visual surface 구조는 통과하지만 `serviceSyncState=delayed`, 첫 카드 `지역별 집회 공개 일정`, 배너 `저장된 공개자료 기준` 상태라 구체 이슈 우선 UX와 운영 live 데이터가 아직 미달이다. visual smoke/service-watch가 첫 이슈와 source-bundle first 여부를 함께 기록하게 했다. | A 후보 Active · API DNS/headers/live 데이터 연결 전 S+ 아님 |
