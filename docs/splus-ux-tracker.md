...
현재 사용자 경험 등급은 A 후보 범위다. 상업용 S+ 재설계는 Active다. 2026-07-11 06:11 KST 독립 재검토는 최신 홈을 Visual C+~A- 후보, 국평오 5초 PASS-/10초 PARTIAL/20초 PARTIAL로 봤다. 26차 개선은 데스크톱 기본 홈의 우측 상세 패널을 닫아 첫 화면을 `이슈 목록 + 지역 현황 지도` 2축으로 단순화했고, 27~28차 개선은 지도 선택 시트·검색·범례·탐색 타일을 compact하게 낮췄다. 사용자 수락 전까지 S+로 표기하지 않는다.

- 사용 가능성: 홈에서 이슈명, 어디서 확인됐는지, 근거 진입이 보이고, 데스크톱 상세에서 영상·지도·근거로 바로 이동할 수 있다.
- 디자인 완성도: A-~A+ 후보
- 국평오 직관성: B-~A+ 후보
- 핵심 문제: 데스크톱 패널 경쟁과 지도 도구 밀도는 1차 완화됐지만, 실제 제보 영상 품질, 독립 재검증, 사용자 수락은 아직 S+가 아니다.
...
| 지도 UX | A+ 후보 | 지도는 자료 위치와 현장 인증 범위만 보여주고 이슈 파일과 연결된다. | Active · 데스크톱 지도 toolbar 54px/sheet 62px, 모바일 지도 toolbar 52px/sheet 114px, 탐색 타일 52px, detail hidden. 모바일 홈은 지도 미노출 |
