...
function nextOperatorPrerequisite(stage) {
  ...
  if (stage === "apply_static_headers") {
    return "Render API token이 있으면 `pnpm launch:apply -- --apply --deploy-web`으로 musunil-web Headers를 적용하고 배포까지 요청한다. Render headers가 live에 계속 없거나 Render token 없이 Web header만 먼저 고치려면 `pnpm launch:apply -- --apply --cloudflare-headers-only`로 Web 전용 Cloudflare fallback을 추가하고, 하위 확인은 `pnpm cloudflare:headers`를 사용한다.";
  }
  if (stage === "publish_build_metadata") {
