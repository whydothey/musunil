...
| 현장 영상 UX | A 후보 | 영상이 확정 결론이 아니라 현장 인증 자료이며, 근거·위치 범위·관련 이슈에 접근한다. | Active · 33차로 poster 없는 LIVE Claim은 검토 카드로 표시하고, 34차로 공개 clip+poster가 모두 있으면 풀스크린 `<video class="reel-video">`로 표시. 실제 공개 영상 파일이 붙은 캡처와 사용자 수락 필요 |
