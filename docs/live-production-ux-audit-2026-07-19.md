...
| G7 | live + CI | `service:watch -- --once --with-visual`: 정적 manifest 16개/2,458,357 bytes와 local hash 일치, Web config의 공개 필드 2개와 `https://api.musunil.com` 일치, 4 viewport 정직한 fallback surface. [GitHub Actions #29682268419](https://github.com/whydothey/musunil/actions/runs/29682268419) 1m 55s 성공 | **Active / blocked** | `build-info.json`이 placeholder, `/`·`config.js`·`build-info.json` 보안 헤더 누락, `api.musunil.com` DNS 미연결, Render/Cloudflare token·API target·runtime Secret File·원천/PortOne/storage/비식별/무결성 credential 미입력 |
