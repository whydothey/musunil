...
- 서울: 집회성 모임, 장기 현장, 교통 통제, 지하철 영향, 인파 신호, 경로 지점
- 부산: 행진 가능성, 도시철도 영향, 주최 없는 인파 밀집, 경로 분기 지점
- 대전: 장기 현장
- 법 탭: 정보통신망법 개정안, 국회법 탄핵 절차, 공직선거법 관련 프리뷰 법령/의안 메타데이터

이 데이터는 UI/UX 검증용이다. 운영에서는 `preview.use_mock_data: false`와 production runtime gate로 비활성화되어야 하며, 실제 서비스 응답에 섞이면 안 된다. 모든 항목은 Event가 아니라 Claim/Evidence로 붙는다. 사용자 원문은 공개 응답에 노출하지 않는다.
