...
- UX·디자인은 사용자 판정과 충돌한 문서상 S+ 선언을 리셋했고, 상업용 앱 수준 재설계가 Active다. 1~30차 개선으로 홈 KPI/숫자판/장식 커버/운영자 언어/지도 숫자판/제보 기술 문구/상세 탭 오표기/모바일 카드 과밀/제보 단계표를 낮췄고, 공개 poster 프리뷰·근거 탭 직행·선택 이슈 문맥 라인·지도-first·컴팩트 지도 시트·하단 근거 도구막대·검토 대기 영상 슬롯·지도 중심 데스크톱 레이아웃·dev config 회귀 방어·홈 compact 검토 행·홈 scanline·데스크톱 홈 지역 현황 지도·기본 홈 상세 닫힘·지도 선택 시트 경량화·지도 도구막대/탐색 타일 경량화·공식 fallback 이슈·지도 선택 현장 상세 동기화·모바일 5탭 overflow 방지·홈 주행동 단일화·`인증영상/현장촬영` 라벨 분리·`위치/현장/공식 자료/인원` scanline을 연결했다. 10:16 독립 재검토는 CTA 분산과 영상/제보 혼선을 핵심 문제로 봤고 30차에서 1차 개선했으므로 아직 S+가 아니다. 남은 리스크는 지도 placeholder감, 실제 제보 영상 품질, 독립 재검증, 사용자 수락이다.
...
| 지도·지역 UX | A+ 후보 | 지도는 보조 확인 수단이며 선택 결과가 카드·상세와 즉시 동기화된다. | `docs/splus-ux-tracker.md` | 사용자 수락, 지도 선택-상세 동기화 유지 |
...
| 2 | 상업용 UI/UX 재설계 | Active | 10:16 독립 재검토에서 홈 CTA 분산, `영상/영상제보` 혼선, 위치·규모 단위 불명확이 남아 있었다. 30차로 홈 주행동을 `지도에서 확인` 1개로 통합하고 보조 액션을 `근거/인증영상/반론`으로 낮췄으며, 하단/좌측 탭을 `인증영상/현장촬영`으로 분리했다. 실제 제보 영상 품질, 지도 placeholder감, 독립 재검증, 사용자 수락은 남아 있다. | 30차 캡처를 기준으로 지도 화면 placeholder감과 박스형 패널 밀도를 줄이고, 실제 공개 영상 표면을 운영 데이터 구조에 맞게 재검증한다. | `docs/commercial-splus-redesign.md`, 30차 home/reels 캡처, primary CTA metrics, web smoke, 사용자 수락 |
...
| 16 | 지도·지역 UX | Active | 지도 탭과 데스크톱 홈 모두 지도 맥락이 올라왔고 기본 홈 지도 폭은 828px로 확대됐다. 29차로 지도 `상세`가 선택 현장 상세로 열리고, 검색 실패는 시트 내부 회복 문구로 표시되며, 모바일 `detailTitle=mapTitle`, `selectedTab=근거`까지 확인했다. | 지도 선택 후 영상 탭, 지도 검색 성공/실패, 긴 제목 시트를 390/430/768/1440px에서 독립 재검증한다. | 390/430/768/1440px browser capture, toolbarMapRatio, sheetMapRatio, `navOverlap=false`, `overflowX=false`, 사용자 수락 |
| 17 | 데스크톱·모바일 디자인 | Active | 문맥 라인, 릴스 겹침, 어두운 영상 poster, 데스크톱 제보 빈 공간, 지도-first, 영상 소셜 레일, 샘플 poster 표시, 홈 대형 검토 썸네일, scanline 혼선, 데스크톱 홈 빈 공간, 패널 경쟁, 지도 시트 과밀, 지도 도구 밀도, 모바일 5탭 overflow, CTA 분산, 영상/촬영 라벨 혼선은 1차 개선됐지만 사용자 수락 전이다. | 30차 캡처로 지도 화면과 모바일 홈이 상업용 앱처럼 읽히는지 재평가하고, 남은 지도 placeholder감과 실제 제보 영상 표면을 낮춘다. | `docs/commercial-splus-redesign.md`, 30차 screenshots, `rejectedTerms=[]`, `overflowX=false`, 사용자 수락 |
