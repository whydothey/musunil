...
| 4 | Issue-first live data | `sourceBundleFirst=4/4`: 390/430/768/1440px 홈 첫 카드가 구체 주제가 아니라 공개자료 묶음이다. API 연결 후 `public_payload_home`도 같은 계약을 검증한다. | `/home.issueCards`의 첫 항목이 실제 주제형 Issue가 되게 하고, 공개자료 묶음은 자료 범위/지역 현황 맥락으로 낮춘다. 실제 주제 ingest 전에는 공개자료 묶음을 이슈 카드로 대체하지 않는다. | `pnpm check:visual-surface:live && MUSUNIL_WEB_BASE_URL=https://musunil.com MUSUNIL_API_BASE_URL=https://api.musunil.com MUSUNIL_EXPECTED_API_BASE_URL=https://api.musunil.com pnpm service:watch:visual` |
