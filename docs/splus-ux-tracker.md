...
| 화면 | 파일 | 확인 |
|---|---|---|
| 상업용 재설계 69차 현재 evidence 세트 | `docs/visual-evidence/current/visual-surface-evidence.json` | `pnpm check:visual-surface:evidence`가 390/430/768/1440px의 홈·상세·영상·탐색·제보 20개 PNG를 생성. 현재 로컬 첫 이슈는 `정보통신망법 개정 관련 집회`, `sourceBundleFirst=false`, forbidden 0, `serviceSyncState=delayed`. 화면상 카드 요약은 표형 라벨 없이 장소·근거 문장으로 표시됨 |
| 상업용 재설계 69차 모바일 홈 | `docs/visual-evidence/current/mobile_390_home.png` | 390px 홈에서 story ring, 이슈 카드 2개, 하단 5탭, 표형 `위치/근거` 라벨 제거, `상세 보기/다른 주장` 경로 유지. API live 동기화 전이라 S+ 완료 증거는 아님 |
| 상업용 재설계 69차 데스크톱 홈 | `docs/visual-evidence/current/desktop_1440_home.png` | 1440px 홈에서 좌측 이슈 피드가 지도보다 먼저 읽히고, 우측 지도는 자료 위치·인증 범위 맥락으로 유지. API live 동기화와 사용자 수락 전 S+ 완료 증거는 아님 |
