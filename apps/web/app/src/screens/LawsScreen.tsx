  ...
  return (
    <section className="screen screen-feed laws-screen" data-screen="laws">
      ...
      <div className="law-list">
        {groups.map((group) => {
          const primaryTopic = group.coreTopics[0]?.label;
          return (
            <Link href={`/laws/groups/${encodeURIComponent(group.id)}`} className="law-row law-topic-row" key={group.id} ariaLabel={`${group.billTitle} 법안 그룹 ${group.billCount}건 보기`}>
              ...
              <div className="law-row-main"><div><h2>{group.billTitle}</h2><p>개별 의안 {group.billCount}건{primaryTopic ? ` · ${primaryTopic}` : ""}</p></div><ChevronRight aria-hidden="true" /></div>
            </Link>
          );
        })}
      </div>
