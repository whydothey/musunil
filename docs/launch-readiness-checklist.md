...
- `pnpm launch:final-gate` 통과. 이 명령은 배포 후 `pnpm sources:refresh-preflight`, `pnpm cloudflare:check:strict`, `pnpm launch:post-deploy-smoke -- --require-laws --require-source-refreshes`, `pnpm launch:blockers:refresh-strict`를 순서대로 실행하고, 앞 단계가 실패해도 live blocker 갱신까지 시도한 뒤 최종 실패한다. `MUSUNIL_RENDER_API_DNS_TARGET`이 없고 Render API token이 있으면 Render API service URL에서 target을 자동 파생해 strict DNS check에 넘긴다.
