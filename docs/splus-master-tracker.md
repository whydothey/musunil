...
> 상세 실행 보드: [live-production-ux-audit-2026-07-19.md](/Users/mk/Documents/Musunil/docs/live-production-ux-audit-2026-07-19.md). React/Vite UI-G1~G6는 실제 `musunil.com` 배포와 4개 viewport 검증까지 Guard다. 다음 순차 active goal은 비용 승인을 보류한 `G7. 실제 운영 연결과 공식 원천 복구`다. 전체 S+ 판정은 이 문서가 아니라 감사 문서의 실제 운영 증거를 따른다.
...
- 2026-07-20 React/Vite 전면 재구축을 실제 `musunil.com`에 배포했다. Render Publish Directory와 SPA rewrite 불일치를 바로잡았고, 라이브 SHA·직접 경로·보안 헤더·390/430/768/1440 빈 상태와 지도 문맥이 통과했다. 독립 QA가 지적한 지도, 릴스 근거 이동, 법안 상세, 제보 전체 단계, 데스크톱 폭도 수정했다. UI 출시 후보는 Guard지만, 실제 운영 이슈·영상·법안·제보 데이터는 API가 없는 현재 검증할 수 없으므로 전체 S+는 아니다.
- UX·디자인은 사용자 판정과 충돌한 문서상 S+ 선언을 리셋했고, 상업용 앱 수준 재설계가 Active다. 1~53차 개선으로 홈 KPI/숫자판/장식 커버/운영자 언어/지도 숫자판/제보 기술 문구/상세 탭 오표기/모바일 카드 과밀/제보 단계표를 낮췄고, 공개 poster 프리뷰·근거 탭 직행·선택 이슈 문맥 라인·지도-first·컴팩트 지도 시트·하단 근거 도구막대·검토 대기 영상 슬롯·지도 중심 데스크톱 레이아웃·dev config 회귀 방어·홈 compact 검토 행·홈 scanline·데스크톱 홈 지역 현황 지도·기본 홈 상세 닫힘·지도 선택 시트 경량화·지도 도구막대/탐색 타일 경량화·공식 fallback 이슈·지도 선택 현장 상세 동기화·모바일 5탭 overflow 방지·홈 주행동 단일화·`인증영상/현장촬영` 라벨 분리·`위치/현장/공식 자료/인원` scanline·지도 fallback 표면 개선·MapLibre layer 재동기화·현장 인증 Evidence 기반 지도 영역 분리·인증 영역 근접 줌·poster 없는 인증영상 검토 카드화·실제 공개 영상 player 계약·공개 media route/CSP 계약·surface50 데스크톱 홈 지도 비중 완화·surface51 poster 없는 홈 영상 썸네일 제거·surface53 주요 이슈 story ring 전환을 연결했다. 2026-07-12 01:29에는 숨겨진 상단 숫자판 DOM/로직을 제거하고 Chrome/CDP 기반 `pnpm check:visual-surface`를 release gate에 추가했다. 01:52에는 `pnpm check:visual-surface:live`와 운영 fallback 3개 공개자료 이슈 파일을 추가했고 새 Render 배포 후 live 검증이 통과했다. 02:09에는 visual smoke가 `serviceSyncState`를 기록하고 service-watch가 운영 도메인의 `delayed` fallback 상태를 실패로 처리하게 했다. 03:37 라이브 캡처는 모바일/데스크톱 구조 안정성은 확인했지만 API 미연결 때문에 첫 이슈가 구체 주제가 아닌 공개자료 묶음으로 보이는 상태를 증거화했다. 11:44에는 모바일 상세 시트의 중복 장문 설명을 숨기고 visual smoke가 상세 상단 높이·버튼 폭·가로 clipping을 검사하게 했다. 19:53에는 poster 없는 LIVE Claim이 더 이상 풀스크린 fake video surface로 보이지 않도록 `reel-pending-card`로 낮추고 visual smoke가 `reelPendingFullCount=0`을 검사하게 했다. 22:35에는 production fallback 첫 이슈를 더 구체적인 `정보통신망법 개정 반대 집회`로 올리고, 주행동을 `근거 보기`로 고정하며, 공개 clip+poster가 있는 경우만 `영상 보기`를 노출하고, 제보 첫 화면에 위치 권한 전 후보 정보 미리보기를 추가했다. 22:51에는 실제 `https://musunil.com` live evidence 20개 화면이 모두 통과했고 첫 이슈도 `정보통신망법 개정 반대 집회`, source bundle first 0/4로 확인됐다. 로컬/정적/live 렌더링 구조는 검증됐지만, 모든 live 화면이 `serviceSyncState=delayed`라 실제 운영 GPS evidence, 실제 제보 영상 품질, live API 동기화, 독립 재검증, 사용자 수락 전에는 S+가 아니다.
...
- 실제 `pnpm storage:smoke` 실행과 실제 `pnpm redaction:smoke` 실행은 각각 R2 read-back과 영상 음성·메타데이터 제거를 통과했다. 운영 S+에는 배포 redaction worker end-to-end, 실제 `pnpm identity:smoke`, 운영 DB, 실제 원천 수집 스케줄, 모바일 앱 무결성 provider dry-run이 남아 있다. Admin redaction bypass는 차단되어 공개본 URL과 redaction proof는 내부 redaction worker만 기록한다.
...
| 법안·개정안 연결 | Guard | 공식 발의일·공포/시행일을 분리하고, 현장 관심/최근 발의 정렬과 이슈·현장 교차 이동을 local에서 검증했다. | `docs/live-production-ux-audit-2026-07-19.md` | 실제 API 키 또는 OC로 dry-run/post와 live feed 검증 |
...
| 지도·지역 UX | Guard | 지도는 자료 위치 핀과 인증 영역만 사용하고 선택 결과가 공통 현장 상세와 동기화된다. 실제 배경 지도와 4개 viewport 문맥 검증이 통과했다. | `docs/live-production-ux-audit-2026-07-19.md` | 실제 운영 GPS evidence 유입 후 영역·현장 동일성 재검증 |
| 데스크톱·모바일 디자인 | Guard | React/Vite 단일 피드·전체 화면 상세·5탭·920px 데스크톱 열을 실제 라이브와 fixture 4개 viewport에서 검증했다. | `docs/live-production-ux-audit-2026-07-19.md` | 실제 운영 데이터가 있는 라이브 5·10·20초 검증 |
...
| 2 | 상업용 UI/UX 재설계 | Guard | React/Vite 전면 재구축, 독립 QA 수정, production fixture 격리, Render dist 게시와 SPA rewrite, 라이브 4개 viewport 검증이 통과했다. 실제 운영 데이터가 없으므로 데이터가 있는 live 5·10·20초 흐름은 남아 있다. | G7 운영 연결 뒤 같은 이슈·현장·영상·법안·제보 흐름을 실제 데이터로 재검증한다. | `docs/live-production-ux-audit-2026-07-19.md`, React candidate 68개 PNG, live-current PNG/JSON, GitHub Actions run 29693678360 |
| 3 | 운영형 현장 인증 | Active | 내부 기기 무결성 기록 경로, redaction proof, admin bypass 차단, trusted device integrity publish gate, 서버 기준 PoP, S3-compatible AES-GCM 원본 저장, 실제 R2 PUT/GET/DELETE, 내장 ffmpeg 영상 비식별 smoke, 내부 queue/worker, 비공개 파생본 API proxy까지 구현됐다. 실제 Render API가 없어 암호화 원본→worker→운영 검토 end-to-end와 모바일 attestation provider 호출은 아직 검증 전이다. | API/DB 생성 뒤 redaction worker와 mobile integrity verifier를 운영 설정으로 end-to-end 검증한다. | 업로드→검토→비식별 공개본→공개 live-claims까지 runtime smoke 통과 |
| 4 | 운영 배포 준비 | Active | Render 환경변수/Secret File, 운영 DB, cron, 모니터링은 사용자 입력값 투입 전이다. LIVE 자동 공개 오설정, production storage 누락, production 법 공개 원천 키 누락, production 모바일 무결성 누락은 launch validation으로 차단되고, runtime not-ready 상태의 production write는 503으로 차단된다. Web은 static hash로 최신 UI 파일 여부를 보조 검증하며 build-info placeholder는 출시 blocker로 처리한다. API 미연결 상태는 `공개자료로 먼저 확인` 배너로 공개 화면에서 명확히 표시한다. Render 수동 Static Site 값은 `pnpm render:web-settings`로 추출하고, `service:watch`는 API DNS/HTTPS preflight, Web header contract, Required Actions를 분리한다. Live UI는 `pnpm check:visual-surface:live`와 `pnpm service:watch:visual`로 검증한다. | 사용자 입력 YAML과 Render secrets를 기준으로 dry-run 없이 실제 런타임 부팅·migration·health check를 통과시킨다. | `pnpm check:release`, `pnpm render:web-settings`, `pnpm service:watch -- --once`, `pnpm check:visual-surface:live`, `pnpm service:watch:visual`, 운영 health/runtime smoke, launch checklist 완료 |
| 5 | 법안·개정안 연결 | Guard | `proposedDate` 분리, 국회 의안 전용 최근 발의 정렬, 공식 도메인 원문 링크 guard, production preview 법 데이터 비노출, 법안 → 이슈/현장 동일성, 4개 viewport 캡처까지 통과했다. | 실제 공개 API key 또는 OC 입력 후 official ingest dry-run/post와 live `/laws` 재검증을 G7에서 수행한다. | API/worker self-check, `check:law-diagnostics`, `check:web-smoke`, `check:web-flow`, Goal 5 visual evidence |
| 6 | 본인확인 기반 쓰기 경계 | Active | local staging에서 위치 후보·대상 확정·test identity·카메라 촬영·미리보기·접수·새로고침 뒤 접수 카드 복원까지 통과했다. API에는 PortOne identity start/complete, verified session, `identity_required` write boundary, Web 인증 시트, launch validation이 들어갔다. 자료 제보, 현장 정정, 권리침해 신고, 반론은 인증 후에도 `held_private` 검수 대기로만 저장되고 공개 detail은 Admin review 전까지 변하지 않는다. 실제 포트원 채널 키와 운영 SDK 리허설은 외부 연결 전이다. | 실제 포트원 본인확인 채널로 인증 완료 리허설을 통과시키고 service-watch에 증거를 남긴다. | Goal 6 visual evidence, API self-check, runtime smoke `identity_required`, write-check public detail invariant, Web smoke identity sheet |
...
| 8 | 개인정보·권리 보호 | Active | 내부 dashboard, 공개 반경, production 원본 메모리 보관 차단, 원본 AES-GCM 암호화, 최소 권한 비공개 R2와 실제 PUT/GET/DELETE, 내장 영상 비식별 smoke, purge 전 외부 delete gate를 통과했다. 운영 API/DB가 없어 실제 worker·보존 삭제·권리 요청 end-to-end는 남아 있다. | 정밀 위치 비공개, worker, 권리 검토, purge를 배포 운영 저장소 기준으로 검증한다. | privacy dashboard, 공개 payload private field 부재, purge self-check |
...
7. 법안·개정안 연결 S+ - Guard
   ...
   완료됨: 국회 의안/국가법령 API 응답 파서 mock self-check, `/laws?sort=interest|proposed_desc`, 공식 발의일 분리, 공식 도메인 link guard, 법안→이슈/집회 현장 교차 이동, production 법 공개 원천 키 누락 launch validation, credential 존재 시 0건 dry-run 실패 가드가 있다.
...
| Launch validation | production에서 LIVE 자동 공개, mock 지도 provider, preview/sample 데이터, placeholder secret, localhost origin을 차단한다. |
...
| live-current 운영 화면 evidence | `docs/visual-evidence/live-current/visual-surface-evidence.json` | 2026-07-12 22:51 KST 실제 `https://musunil.com` 기준 최신 static/config 화면을 390/430/768/1440px 홈·상세·영상·탐색·제보 20개 PNG/JSON으로 남겼다. 화면 구조는 모두 통과했고 첫 이슈는 `정보통신망법 개정 반대 집회`, source bundle first는 0/4다. 단 API DNS 미연결로 `serviceSyncState=delayed`라 API live Issue 연결 전 S+ 아님 |
...
| 2026-07-10 04:47 KST | 운영형 현장 인증 | `mobile.integrity_smoke_command`와 `pnpm mobile:integrity-smoke`를 추가하고 `pnpm launch:external-smoke`에 연결했다. 실제 provider dry-run은 `mobile_integrity_provider_dry_run` structured proof를 출력해야 한다. | 모바일 attestation dry-run 게이트 준비 |
...
| 2026-07-11 21:26 KST | 운영 배포 준비 | `check:web-deploy`와 `service:watch`에 static hash 검증을 추가했다. live HTML/config/media SHA-256이 현재 repo manifest와 같으면 최신 UI 파일은 확인되지만, `/build-info.json` placeholder와 no-store header 미적용은 출시 blocker로 남긴다. static hash가 다르면 실패한다. | Active · 운영 API/원천/인증/저장소와 사용자 수락 전 완료 아님 |
...
| 2026-07-12 05:12 KST | 운영 배포 준비 | launch config validation이 `app.public_base_url`/`api.public_base_url`의 배포 HTTPS 여부, `web.allowed_origins`의 정확한 Origin 형식과 app origin 포함 여부, 본인확인 API의 host-only 쿠키 여부를 검사하게 했다. API 연결 뒤 CORS·본인확인·제보 흐름이 설정 오타로 깨지는 것을 출시 전에 차단한다. | Guard 강화 · 실제 API DNS/PortOne 리허설 전 완료 아님 |
...
| 2026-07-12 18:25 KST | 운영 저장소 안전 | storage smoke key 제한을 `assertStorageSmokeKey` 모듈 함수로 승격하고 API self-check가 정상 smoke key, 기존 원본 media key 후보, traversal, double-slash 사례를 직접 검증하게 했다. `launch-check`는 구현·CLI 사용·self-check 사례를 함께 감시한다. | Guard 강화 · 정적 문자열 감시에서 실행 가능한 회귀 테스트로 강화, 실제 storage credential smoke 통과 전 완료 아님 |
| 2026-07-12 18:33 KST | 비식별 smoke 안전 | `redaction:smoke`가 샘플 얼굴/차량번호 토큰을 그대로 남기는 command를 실패 처리하게 했다. `check:redaction-smoke-safety`는 redacted fixture 통과와 copy fixture 실패를 실제 실행하고, release gate와 `launch-check`가 이 계약을 감시한다. | Guard 강화 · 단순 copy command를 비식별 엔진 증거로 오판하는 회귀 방지, 실제 redaction provider smoke 통과 전 완료 아님 |
| 2026-07-12 18:41 KST | 모바일 무결성 smoke 안전 | `mobile:integrity-smoke`가 marker 문자열만 출력하는 command를 거부하고 provider, packageName 또는 bundleId/teamId, verdict가 들어간 structured proof JSON을 요구하게 했다. `check:mobile-integrity-smoke-safety`는 정상 proof, marker-only, wrong-package, secret-leak fixture를 실제 실행하고 release gate에 포함된다. | Guard 강화 · marker-only command를 Play Integrity/App Attest dry-run 증거로 오판하는 회귀 방지, 실제 provider smoke 통과 전 완료 아님 |
| 2026-07-12 19:20 KST | 운영 원천 오입력 방지 | README의 보류 항목에서 `공개 집회/교통 원천` 표현을 `공개 집회·시위 원천`으로 정리하고, `launch-check`가 README에서도 비시위 원천 표현 회귀를 잡게 했다. 같은 handoff에서 live blocker report와 운영 브리프/입력 체크리스트 시각을 다시 맞췄다. | Guard 강화 · 일반 교통 데이터 연결 오판 방지, 실제 DNS/API/Web header/live sync 전 완료 아님 |
| 2026-07-12 19:25 KST | 운영 CI 상태 확인 | `pnpm ci:status`를 추가해 현재 Git SHA의 GitHub Actions `ci.yml` run을 `queued`/`in_progress`/`completed`로 구분하고, 정확한 `gh run watch` 명령을 출력하게 했다. `launch-check`와 출시 문서가 이 helper를 감시하며, 최신 커밋 `e5eeb46`의 run `29188939235`는 `completed/success`로 확인됐다. | Guard 강화 · push CI 상태 오판 방지, 실제 DNS/API/Web header/live sync 전 완료 아님 |
| 2026-07-12 19:32 KST | 운영 handoff CI 확인 | `launch:operator-brief` 생성물이 `pnpm ci:status`를 직접 안내하게 했다. 컷오버 런북도 handoff 직후 push CI 상태 확인을 넣고, `launch-check`가 README/런북/운영 브리프에서 이 절차가 빠지면 실패하게 했다. | Guard 강화 · 운영자가 queued CI를 실패나 성공으로 오판하는 위험 감소, 실제 DNS/API/Web header/live sync 전 완료 아님 |
| 2026-07-12 19:43 KST | 상업용 홈 UI 비평 반영 | 독립 Visual Critique/IA Red-Team의 P0를 반영해 홈 제목을 `공개자료 기반 집회·시위`로 낮추고, story fallback을 생성형 일러스트 대신 추상 상태 마크로 바꿨다. 이슈 카드 nested 위치 박스는 평면 `지도에서 보기` 행으로 낮추고, primary CTA는 `근거·영상`으로 변경해 상세 근거 탭에 바로 연결한다. | A+ 후보 Active · `pnpm check:visual-surface:evidence` 통과, 실제 live API 동기화와 사용자 수락 전 S+ 아님 |
| 2026-07-12 20:39 KST | 운영 seed 경계 | 개발용 예시 현장 ID를 `_mock`에서 `_sample`로 정리하고 production strip, Web fallback filter, service-watch public payload gate가 `_mock`/`_sample`/`preview-only` 노출을 모두 실패 처리하게 했다. | Guard 강화 · 실제 live API 동기화와 사용자 수락 전 S+ 아님 |
| 2026-07-12 20:47 KST | 운영 입력 체크리스트 | `pnpm launch:verify-inputs`가 값 검증 전에 로컬 YAML과 template 필드 구조를 비교하게 했다. 오래된 `config/musunil.user-inputs.local.yaml`에 새 identity/storage/provider 섹션이 빠지면 누락 경로를 먼저 출력한다. | Guard 강화 · 실제 외부 secret/provider 입력 전 완료 아님 |
| 2026-07-12 20:58 KST | 운영 배포 준비 | Render Static 전용 build command에 `pnpm check:web-render-build-output`을 추가했다. 이 검사는 `config.js`가 `https://api.musunil.com`을 가리키는지, `build-info.js/json`이 tracked placeholder가 아닌 실제 commit metadata인지, `_headers`와 static manifest가 배포 산출물 계약을 만족하는지 확인하고 실패 시 Render build를 중단한다. | Guard 강화 · 올바른 Render build command 사용 시 build-info placeholder 재배포 방지, 실제 Web headers/API DNS/live sync 전 완료 아님 |
| 2026-07-12 21:05 KST | 운영 배포 준비 | `pnpm check:release`에 `pnpm check:web-render-build-command`를 추가했다. CI가 정확한 Render Static build command를 실제 실행하고, 테스트 중 생성된 `build-info`·config·manifest·headers tracked 산출물은 자동 복원하므로 Render 전용 guard의 문법/동작 오류를 배포 전 잡는다. | Guard 강화 · CI에서 Render build 계약 실행 검증, 실제 Render/Cloudflare/API 연결 전 완료 아님 |
| 2026-07-12 21:12 KST | 운영 배포 준비 | README, readiness checklist, `pnpm render:web-settings` 출력에 `pnpm check:web-render-build-command` 사전 실행을 연결했다. 운영자가 Render Dashboard 값을 바꾸기 전 같은 build contract를 로컬에서 먼저 확인하고, `launch-check`가 이 안내가 빠지면 실패한다. | Guard 강화 · 수동 Render 설정 전 로컬 preflight 누락 방지 |
| 2026-07-12 21:14 KST | 운영 배포 준비 | `service:watch`, `launch:blockers`, `launch:cutover-plan`, `launch:cutover-rehearsal`의 `deploy_latest_static`/`publish_build_metadata` 안내가 모두 `pnpm check:web-render-build-command`를 먼저 실행하도록 수렴했다. `launch-check`가 이 문구를 감시하므로 operator-facing helper가 Render Dashboard 수정을 local preflight 없이 안내하는 회귀를 막는다. | Guard 강화 · Render 수동 설정 전 사전검증 누락 방지, 실제 Render/Cloudflare/API 연결 전 완료 아님 |
| 2026-07-12 21:18 KST | 운영 배포 준비 | `ci-launch-check`와 `ci-web-render-build-command`가 `tmp/web-static-files.lock`을 공유하게 했다. 두 검사가 병렬로 실행되어도 `apps/web/config.js`, build-info, `_headers`, static manifest를 서로 덮어읽지 않으며, stale lock은 자동 제거된다. | Guard 강화 · 로컬 병렬 검증으로 인한 Render build 계약 오판 방지 |
| 2026-07-12 21:21 KST | 운영 런북 정합성 | `docs/launch-cutover-runbook.md`의 현재 차단 항목을 최신 live blocker와 맞췄다. 런북은 더 이상 `web_build_info`가 통과했다고 말하지 않고, build metadata 해소 전에 `pnpm check:web-render-build-command`를 먼저 실행하라고 안내한다. `launch-check`가 런북에서 stale build-info 통과 문구와 preflight 누락을 실패 처리한다. | Guard 강화 · operator-facing 런북 stale 상태 오판 방지, 실제 Render/Cloudflare/API 연결 전 완료 아님 |
| 2026-07-12 21:25 KST | 후배포 검증 | GitHub Actions `post-deploy` 수동 workflow가 live 검증 전에 `pnpm check:web-render-build-command`를 먼저 실행하게 했다. Render 배포 후 원격 `web-deploy`/`final-gate` 검증은 이제 배포 대상 커밋의 Render Static build contract와 live Web/API 상태를 같은 workflow run에서 확인한다. `launch-check`가 workflow와 운영 문서의 이 preflight 안내를 감시한다. | Guard 강화 · post-deploy 단독 실행 시 Render build contract 누락 방지, 실제 workflow 실행/DNS/API/live sync 전 완료 아님 |
| 2026-07-12 21:50 KST | 운영형 현장 인증 경계 | 공용 스키마에서 LIVE 영상 Proof-of-Presence와 현장 판단용 sensor signal을 분리했다. `hasProofOfPresence`는 `live_media + in_app_camera + 최소 5초`만 통과하고, sensor는 `hasFieldPresenceSignal`로만 판정한다. schema/API self-check와 `launch-check`가 sensor를 영상 제보 proof로 인정하는 회귀를 차단한다. | Guard 강화 · 현장 판단은 유지하되 영상 제보 우회 방지, 실제 provider smoke 전 완료 아님 |
| 2026-07-12 21:57 KST | 운영 증거 신뢰성 | `launch:blockers`가 stale live report를 감지하면 상단에 `STALE LIVE EVIDENCE`를 표시하고, Split Apply Paths/Blocking Checks/Required Actions를 refresh 전 진단 참고로만 표시하게 했다. JSON에는 `staleDecisionWarning`과 `actionsAdvisoryOnly`를 노출하고 `launch-check`가 이 계약을 감시한다. | Guard 강화 · 오래된 live blocker snapshot으로 Render/Cloudflare 변경이나 완료 판정을 하는 오판 방지 |
| 2026-07-12 22:06 KST | 운영 증거 신뢰성 | `launch:blockers`의 stale live evidence 경고를 `launch:cutover-rehearsal`, `launch:operator-brief`, `launch:missing-inputs`까지 전파했다. 브리프와 입력 체크리스트만 보는 상황에서도 Apply/Required Action은 refresh 전 진단 참고로 표시되고, `launch-check`가 이 전파 경로를 감시한다. | Guard 강화 · 오래된 운영 브리프/입력 체크리스트로 외부 설정을 변경하는 오판 방지 |
| 2026-07-12 22:09 KST | 운영 증거 신뢰성 | `service:watch`가 live static manifest timeout을 실제 manifest mismatch와 분리한다. timeout이면 `retry_live_static_manifest`로 재검증을 먼저 요구하고, schema/hash mismatch가 확인될 때만 `deploy_latest_static`을 안내하므로 transient network 실패를 Render 재배포 지시로 오판하지 않는다. | Guard 강화 · 외부 네트워크 흔들림과 실제 구버전 배포 구분 |
| 2026-07-12 22:16 KST | 운영 배포 준비 | `launch:blockers`, `launch:cutover-rehearsal`, `launch:operator-brief`, `launch:missing-inputs`가 외부 설정 변경 전 `Pre-External-Change Checks`를 노출한다. 현재 blocker 기준으로 Render static build contract, Web header-only dry-run, Render/Cloudflare apply dry-run을 먼저 보여주고 `launch-check`가 이 섹션을 감시한다. | Guard 강화 · Render/Cloudflare 화면을 열기 전 로컬 계약과 누락 입력 확인 누락 방지 |
| 2026-07-12 22:35 KST | 상업용 홈·제보 UX | 독립 Visual Critique/IA Red-Team의 P0를 반영해 production fallback 첫 이슈를 `정보통신망법 개정 반대 집회`로 구체화하고, 홈/지도 CTA를 `근거 보기`로 고정했다. 공개 가능한 clip+poster가 있을 때만 `영상 보기`를 보이며, 검토 중 촬영자료는 `촬영자료 n건 검토 중`으로 분리한다. 주요 이슈 레일은 poster 없는 경우 텍스트형 이슈 칩으로 낮추고, 제보 첫 화면은 위치 권한 전에도 후보 선택에서 확인할 정보 3가지를 보여준다. | A+ 후보 Active · `pnpm check:web-smoke`, `pnpm check:web-flow`, `pnpm check:ux-surface`, `pnpm check:visual-surface:evidence` 통과, 실제 live API 동기화와 사용자 수락 전 S+ 아님 |
| 2026-07-12 22:51 KST | 상업용 live UX 증거화 | `pnpm check:visual-surface:live:evidence`로 실제 `https://musunil.com` 390/430/768/1440px 홈·상세·영상·탐색·제보 20개 화면 PNG/JSON을 생성했다. live 첫 이슈는 `정보통신망법 개정 반대 집회`, source bundle first는 0/4이며 모든 시각 상태가 통과했다. | Active · `serviceSyncState=delayed`라 API live 동기화 전 완료 아님 |
| 2026-07-21 KST | 근거 기반 주제 모델 | 법안 그룹별 우산 Issue에 버전·근거 기간·Claim/Evidence ID·발행사 수·세부 논점을 담는 `IssueSynthesisSnapshot`을 추가했다. 홈과 상세는 중립 요약, 논점, 근거·발행사 수를 표시하고 단순 법안 목록 대신 관련 법안 그룹을 보여준다. | 구현·자동 테스트 완료 · 실제 배포 데이터 재합성 확인 필요 |
| 2026-07-21 KST | 이벤트-주제 연결 | 단일 `Occurrence.issueId` 호환 필드를 유지하면서 승인/후보/거절, 연결 근거와 지원 Claim/Evidence를 기록하는 다대다 `OccurrenceIssueLink`를 추가했다. 복수 후보는 비공개 보류하고 이벤트 자체 근거가 하나의 주제와 맞을 때만 자동 승인한다. | 구현·자동 테스트 완료 · 운영 후보 검토 UI는 후속 운영 도구 범위 |
| 2026-07-21 KST | 법안·커버리지 의미 정정 | 법안마다 핵심 논점과 분류 근거를 저장하고 그룹 상세에 논점 필터·15건 페이지 조회를 추가했다. `현장 관심`에서 발의일 가중치를 제거해 `관련 주제 활성도`로 바꾸고, 소스 접근·이벤트 추출·좌표 확인·지도 표시·파서 빈 결과를 별도 지표로 분리했다. | 구현·자동 테스트 완료 · 전국 첨부파일 이벤트 추출 확대 필요 |
| 2026-07-21 KST | 안전한 공개·운영 게이트 | 제보 준비 상태를 공개 읽기/제보/운영 게이트로 분리하고, 네이티브 무결성 검증 전 웹 영상 접수를 명시적으로 비활성화했다. 방법론·투명성·개인정보·정정/권리 안내와 API ETag/짧은 캐시, 정적 해시 자산 immutable 캐시를 추가했다. | 구현·자동 테스트 완료 · PortOne/Play Integrity/운영 secret 입력과 Android 앱 출시 전 제보 기능 완료 아님 |
