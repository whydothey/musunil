...
| G6. 현장 제보의 확신 가능한 완료 흐름 | **Guard** | 위치 기반 후보·대상 확정·본인확인·앱 내 촬영·미리보기·대상 변경·접수·새로고침 뒤 접수 상태를 local staging API와 브라우저에서 통과했다. 운영 비공개 저장소 PUT/DELETE도 통과했으며, 실제 PortOne·비식별·모바일 무결성 리허설은 G7에서 수행한다. |
...
| 2026-07-19 20:53 KST | G2/G7 | Linux CI의 390×844 홈에서 이슈 카드가 DOM에는 생성됐지만 첫 viewport 아래로 밀려 실제 가시 카드가 0건이었음 | 모바일에서 앱 헤더와 중복되는 홈 설명 헤더를 숨겨 스토리 이슈 다음에 첫 이슈 카드가 바로 보이게 했다. 시각 smoke는 DOM 개수 대신 실제 viewport 교차와 opacity를 기다리고 실패 시 첫 카드 rect를 기록한다. | `check:visual-surface`, `check:report-flow`, GitHub Actions 재검증 필요 |
...
| G7 | live + CI + Cloudflare R2 | Render Static Site가 실제 Git SHA를 기록해 배포되고 Cloudflare Web 전용 rule이 6개 보안 헤더를 실제 응답에 적용한다. schema 3 manifest의 안정 파일은 local commit과 일치하며 변동 파일을 포함한 16개 live 파일도 자체 hash를 통과한다. Web DNS, HTTPS, runtime config, 금지 UI, strict header smoke가 통과했다. 비공개 R2 bucket과 최소 권한 credential의 실제 PUT/DELETE도 통과했다. Postgres lease 기반 단일 ops scheduler 계약도 local Guard다. | **Active / external blocker** | `api.musunil.com`은 NXDOMAIN이고 Render API/DB/Redis가 아직 없다. 이 때문에 live visual은 `serviceSyncState=delayed`다. scheduler의 실제 lease/재시도, 공식 법 원천, PortOne, 비식별, 모바일 무결성 credential과 smoke가 남아 있다. |
...
- Cloudflare R2 비공개 저장소와 실제 PUT/DELETE는 검증 완료했다. 국회·법제처 원천 키, PortOne, 비식별, 모바일 무결성 운영 credential은 아직 입력되지 않았다.
...
| 항목 | 실제 관찰 | 판정 |
| --- | --- | --- |
| 정적 웹 | `https://musunil.com`은 200으로 응답하고, 안정 정적 파일과 배포 SHA는 로컬 `main`의 `1ed003a302403fd616cd2a6d6cdb8d0361a4f9e8`과 일치 | 최신 정적 화면은 배포됨 |
