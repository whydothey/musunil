...
| 항목 | 현재 등급 | S+ 기준 | 상태 |
|---|---:|---|---|
| 목적 인지 | A 후보 | 첫 5초 안에 “집회·시위 객관 확인 앱”과 “어떤 구체 이슈가 확인됐나”가 바로 보인다. | Active · 로컬/preview는 구체 이슈로 시작하지만 live는 API 미연결로 `issues=0`, `first=none`. 사용자 수락 전 S+ 아님 |
...
| 2026-07-12 04:35 KST | API 미연결 빈 피드 UX | live API 또는 `/home.issueCards`가 비어 있을 때 홈이 빈 리스트처럼 보이지 않도록 `이슈를 불러오지 못했습니다` controlled empty state와 `다시 확인/탐색 보기` 회복 경로를 추가했다. visual smoke는 이슈 0개 실패 detail에 empty state와 action 존재 여부를 함께 기록한다. | Active · API live 연결 전 S+ 아님 |
