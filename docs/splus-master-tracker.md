...
Last updated: 2026-07-12 11:47 KST
...
| 2026-07-12 12:11 KST | 운영 배포 준비 | `pnpm cloudflare:apply`를 추가해 Cloudflare DNS Records와 Response Header Transform Rule을 dry-run 우선으로 생성/갱신할 수 있게 했다. 실제 적용은 `CLOUDFLARE_API_TOKEN`, `CLOUDFLARE_ZONE_ID`, Render target env와 `--apply`가 있을 때만 수행하며, API 레코드는 DNS only로 유지한다. | Guard 강화 · 사용자가 마지막 값만 넣으면 DNS/Header 외부 조치를 자동화할 경로 확보 |
