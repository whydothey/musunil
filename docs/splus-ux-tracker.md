...
| 이슈 중심 흐름 | A 후보 | 이슈 선택 → 지도/근거/인증영상/반론이 같은 이슈 맥락으로 이어진다. | Active · 홈 주행동은 `지도에서 확인` 1개, 보조 액션은 `근거/인증영상/반론`으로 검증됨 |
...
| 제보 UX | A 후보 | 첫 제보자가 위치 확인, 대상 확정, 촬영, 미리보기, 접수 상태를 한 흐름으로 이해한다. | Active · 탭 라벨 `현장촬영`, 첫 CTA `근처 현장 찾기`, GPS 전 단계표/칩/후보/대상/미리보기 숨김. 독립 검증 필요 |
...
| 모바일 UX | A+ 후보 | 390px 기준 가로 넘침 없이 이슈 2개, 위치·근거 문장, 핵심 행동이 보인다. 실제 공개 영상이 없으면 가짜 썸네일을 만들지 않는다. | Active · 390/430/768px `overflowX=false`, scanline clipped false, 홈 지도 미노출, visible issue cards 2, `인증영상/현장촬영` 라벨 |
