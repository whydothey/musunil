...
| --- | --- | --- | --- | --- |
| 2026-07-20 | G5 | 법제처 검색 결과가 1건일 때 배열이 아닌 단일 객체로 내려와 공직선거법·국회법이 누락됨 | 법 원천 parser가 다건 배열과 단일 법령 객체를 모두 읽도록 보완하고, 공식 응답을 두 번 조회해 최초 적재·멱등 재수집·동일 ID 갱신·공식값 복원을 검증하는 `check:laws-live`를 추가했다. | 법제처 공식 live 응답 8건, 필수 법령 3종 확인, 재적재 8건, 중복 0건, update/restore 통과. 국회 의안 API는 key 미입력으로 별도 blocker 유지 |
...
| 2026-07-19 22:44 KST | G7 | 로그인 완료 뒤 실제로 즉시 적용 가능한 외부 연결과 아직 사용자 입력이 필요한 provider가 구분되지 않음 | owner-only 운영 YAML과 live DNS를 다시 감사해 적용 blocker를 분리했다. API cutover에는 Render target source와 Cloudflare token이 필요하고, 출시 provider 검증에는 법 원천, PortOne, Android Play Integrity 값이 남는다. 기존 저장소와 비식별 엔진은 현재 credential로 재검증했다. | 로컬 YAML mode `0600`, `launch:verify-inputs`의 필드명 전용 진단, `launch:blockers -- --refresh`, `cloudflare:check`, `launch:apply` dry-run, 승인 없는 `render:provisioning-plan --require-approved` 차단 확인. 실제 `storage_put_get_delete` read-back/delete와 `redaction_engine_smoke`의 audio/metadata 제거·visual detail ratio 0.546 통과 |
...
| G5 | local + official live source | `proposedDate` additive API, `/laws?sort=interest|proposed_desc`, trusted official URL guard, API/worker parser self-check, 법제처 공식 live 응답 8건과 재수집 중복 0건, 390/430/768/1440 법안 정렬 및 법안 → 동일 집회 현장 이동 캡처 | Guard | 국회 API key 입력 후 실제 발의 의안 수집과 운영 API post, live `/laws` 두 정렬·공식 링크 재검증은 G7에서 수행 |
...
- Cloudflare R2 비공개 저장소의 실제 PUT/GET/DELETE와 내장 ffmpeg 비식별 smoke는 검증 완료했다. 운영 API가 없으므로 암호화 원본 → worker → 비공개 파생본 → API proxy의 배포 end-to-end 검증은 남아 있다. 법제처 공식 sample 기반 live 수집은 검증했지만 운영 OC와 국회 의안 API key, PortOne, 모바일 무결성 운영 credential은 아직 입력되지 않았다.
- 단일 ops scheduler는 local 계약만 통과했다. 실제 Render Postgres에서 동시 claim이 한 번만 성공하고 실패 task가 retry 후 회복되는지는 운영 DB 생성 뒤 검증해야 한다.
- G5의 production law feed는 운영 credential 없이 비어 있어야 하며, 현재 production fallback이 그 원칙을 지킨다. 법제처 live 수집·upsert는 로컬에서 통과했지만 실제 발의 의안은 국회 API key와 운영 ingest 성공 이후에만 표시할 수 있다.
...
- **LawItem**은 공식 의안/법령 참조다. Claim이나 Event가 아니며, 동일 공식 법안명 `LawGroup`에 속한다. 이슈 연결은 승인된 `IssueLawGroupLink`로만 그룹에 연결한다.
...
카드에는 동일 공식 법안명, 의안 수, 단계, 핵심 논점을 표시한다. 그룹 상세에서 승인된 연결 이슈와 개별 의안을 분리해 제공하고, 개별 의안 상세는 공식 메타데이터와 소속 그룹만 표시한다. 어떤 이슈와 연결되는지는 별도의 `IssueLawGroupLink`로 설명 가능해야 하며, “법안이 곧 집회의 진실”처럼 보이면 안 된다.
