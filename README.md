...
이 명령은 `musunil.com`과 `api.musunil.com`의 DNS, Web HTTPS, `config.js`의 `apiBaseUrl`, 핵심 Web 보안 헤더, API `/health`, `/ready`를 한 번에 보여준다. 최종 게이트로 쓸 때는 `pnpm cloudflare:check:strict`를 실행한다.
