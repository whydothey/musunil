...
Render 무료 Web Service는 Pre-Deploy Command를 제공하지 않으므로 API Docker 컨테이너가 시작될 때 `pnpm db:migrate`를 먼저 실행한다. 마이그레이션 SQL은 반복 실행 가능해야 하며, 성공한 뒤에만 API 프로세스를 시작한다. Blueprint의 `preDeployCommand`는 유료 플랜이나 이후 전환 시에도 같은 계약을 유지하기 위한 이중 안전장치다.
