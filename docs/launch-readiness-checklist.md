...
- GitHub Actions `post-deploy` 수동 workflow 실행. Render Web 배포 직후에는 `web-deploy` 모드로 strict Web header/static hash를 확인하고, API DNS/Header/원천 연결 뒤에는 `final-gate` 모드로 `pnpm launch:final-gate`를 원격에서 실행한다. 이 workflow는 선택적으로 repository/environment secret `RENDER_API_TOKEN` 또는 `MUSUNIL_RENDER_API_TOKEN`, `MUSUNIL_INTERNAL_API_KEY`를 읽는다. Render API token을 workflow secret으로 쓰지 않는 경우에는 Render `api.musunil.com` custom-domain DNS target hostname을 `render_api_dns_target` 입력값으로 넣어 strict API CNAME 검증까지 실행한다. 이 workflow는 push 자동 실행이 아니며 외부 연결 전 커밋 CI를 막지 않는다.
