...
- UX·디자인은 사용자 판정과 충돌한 문서상 S+ 선언을 리셋했고, 상업용 앱 수준 재설계가 Active다. 1~28차 개선으로 홈 KPI/숫자판/장식 커버/운영자 언어/지도 숫자판/제보 기술 문구/상세 탭 오표기/모바일 카드 과밀/제보 단계표를 낮췄고, 공개 poster 프리뷰·근거 탭 직행·선택 이슈 문맥 라인·지도-first·영상제보 라벨·컴팩트 지도 시트·하단 근거 도구막대·검토 대기 영상 슬롯·지도 중심 데스크톱 레이아웃·dev config 회귀 방어·홈 compact 검토 행·홈 scanline·데스크톱 홈 지역 현황 지도·기본 홈 상세 닫힘·지도 선택 시트 경량화·지도 도구막대/탐색 타일 경량화를 연결했다. 06:11 독립 평가는 최신 홈을 Visual C+~A- 후보, 국평오 5초 PASS-/10초 PARTIAL/20초 PARTIAL로 평가했으므로 S+가 아니다. 남은 리스크는 독립 재검증, 실제 제보 영상 품질, 사용자 수락이다.
...
| 2 | 상업용 UI/UX 재설계 | Active | 06:11 독립 재검토에서 Visual은 C+~A- 후보, 국평오는 5초 PASS-/10초 PARTIAL/20초 PARTIAL이다. 28차로 데스크톱 기본 홈의 패널 경쟁, 지도 시트 과밀, 탐색 타일/지도 도구 밀도는 1차 완화됐지만 실제 제보 영상 품질, 독립 재검증, 사용자 수락이 남아 있다. | 28차 캡처를 기준으로 독립 재검증을 받고, 남은 박스형 패널 밀도와 실제 제보 영상 표면을 줄인다. | `docs/commercial-splus-redesign.md`, 28차 map tools 캡처, action flow metrics, web smoke, 사용자 수락 |
...
| 16 | 지도·지역 UX | Active | 지도 탭과 데스크톱 홈 모두 지도 맥락이 올라왔고 기본 홈 지도 폭은 828px로 확대됐다. 28차로 홈/전용 지도 시트 62px, 모바일 지도 시트 114px, 검색/범례 통합 도구막대, 52px 탐색 pill까지 낮췄지만 사용자 수락은 남아 있다. | 지도 선택 후 상세·영상·근거 흐름을 5탭 이내로 재검증하고, 긴 제목·검색 실패 상태에서도 지도 도구와 선택 시트 120px 이하 조건을 통과시킨다. | 390/430/768/1440px browser capture, toolbarMapRatio, sheetMapRatio, `navOverlap=false`, `overflowX=false`, 사용자 수락 |
| 17 | 데스크톱·모바일 디자인 | Active | 문맥 라인, 릴스 겹침, 어두운 영상 poster, 데스크톱 제보 빈 공간, 지도-first, 영상 소셜 레일, 샘플 poster 표시, 홈 대형 검토 썸네일, scanline 혼선, 데스크톱 홈 빈 공간, 패널 경쟁, 지도 시트 과밀, 지도 도구 밀도는 개선됐지만 사용자 수락 전이다. | 28차 캡처로 지도 화면이 상업용 앱처럼 읽히는지 재평가하고, 남은 박스형 패널 밀도와 실제 제보 영상 표면을 낮춘다. | `docs/commercial-splus-redesign.md`, 28차 screenshots, `rejectedTerms=[]`, `overflowX=false`, 사용자 수락 |
